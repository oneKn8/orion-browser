...
- Avoid magic constants scattered in the codebase. Use `@orion/shared` for all shared configuration:
  - `@orion/shared/constants/ports` - Port numbers (DEFAULT_PORTS, TEST_PORTS)
  - `@orion/shared/constants/timeouts` - Timeout values (TIMEOUTS)
  - `@orion/shared/constants/limits` - Rate limits, pagination, content limits (RATE_LIMITS, AGENT_LIMITS, etc.)
  - `@orion/shared/constants/urls` - External service URLs (EXTERNAL_URLS)
  - `@orion/shared/constants/paths` - File system paths (PATHS)
  - `@orion/shared/types/logger` - Logger interface types (LoggerInterface, LogLevel)
...
**Orion Server** - The automation engine inside Orion. This MCP server powers the built-in AI agent and lets external tools like `claude-code` or `gemini-cli` control the browser. Starts automatically when Orion launches.
...
# Testing
bun run test                     # Run tool tests (requires Orion running)
...
- `tools/` - Tool tests (require Orion running with CDP)
...
```bash
export ORION_CDP_PORT=<port from output>
```
