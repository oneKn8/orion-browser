...
func BuildEnv(p Ports, nodeEnv string) []string {
	env := os.Environ()
	env = append(env,
		fmt.Sprintf("ORION_CDP_PORT=%d", p.CDP),
		fmt.Sprintf("ORION_SERVER_PORT=%d", p.Server),
		fmt.Sprintf("ORION_EXTENSION_PORT=%d", p.Extension),
		fmt.Sprintf("VITE_ORION_SERVER_PORT=%d", p.Server),
