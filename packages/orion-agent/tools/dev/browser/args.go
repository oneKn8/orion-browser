...
func BuildArgs(cfg ArgsConfig) []string {
	binary := "/Applications/Orion.app/Contents/MacOS/Orion"
	...
	if cfg.LoadDevExtensions {
		args = append(args, "--disable-orion-extensions")
	} else {
	...
	args = append(args,
		...
		fmt.Sprintf("--orion-mcp-port=%d", cfg.Ports.Server),
		fmt.Sprintf("--orion-extension-port=%d", cfg.Ports.Extension),
