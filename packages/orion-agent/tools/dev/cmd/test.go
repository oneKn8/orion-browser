...
func init() {
	...
	testCobraCmd.Flags().BoolVar(&testHeadless, "headless", false, "Run Orion headless")
...
func runTest(cmd *cobra.Command, args []string) error {
	...
	if n := proc.CleanupTempDirs("orion-test-"); n > 0 {
	...
	proc.LogMsg(proc.TagBrowser, "Starting Orion...")
	procs = append(procs, proc.StartManaged(ctx, &wg, proc.ProcConfig{
		Tag:     proc.TagBrowser,
		Dir:     root,
		Restart: false,
