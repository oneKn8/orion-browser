 ...
 *
 * Options:
 *   --provider  AI provider (default: orion)
 *   --model     Model name (default: default)
 *   --port      Server port (default: $ORION_SERVER_PORT or 9100)
...
function parseArgs(): {
  ...
  let provider = 'orion'
  let model = 'default'
  let port = process.env.ORION_SERVER_PORT || '9100'
  ...
  if (!message) {
    ...
    console.error(
      '  --port=<port>          Server port (default: $ORION_SERVER_PORT or 9100)',
    )
