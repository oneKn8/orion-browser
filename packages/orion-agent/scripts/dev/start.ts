...
const ORION_BINARY = '/Applications/Orion.app/Contents/MacOS/Orion'
...
let USER_DATA_DIR = '/tmp/orion-dev'
...
Modes:
  --watch     Hot reload mode — wxt launches Orion with HMR, server starts after CDP is ready
  --manual    Manual mode — launches Orion directly, builds both extensions, no HMR
...
function loadEnvPorts(): Ports {
  ...
  return {
    cdp: Number(env.ORION_CDP_PORT) || 9005,
    server: Number(env.ORION_SERVER_PORT) || 9105,
    extension: Number(env.ORION_EXTENSION_PORT) || 9305,
  }
}
...
function createEnv(ports: Ports): NodeJS.ProcessEnv {
  return {
    ...
    ORION_CDP_PORT: String(ports.cdp),
    ORION_SERVER_PORT: String(ports.server),
    ORION_EXTENSION_PORT: String(ports.extension),
    VITE_ORION_SERVER_PORT: String(ports.server),
  }
}
...
function startWatchMode(env: NodeJS.ProcessEnv): ReturnType<typeof spawn> {
  ...
  return spawn({
    cmd: ['bun', 'run', '--filter', '@orion/agent', 'dev'],
    cwd: MONOREPO_ROOT,
    stdout: 'pipe',
    stderr: 'pipe',
    env,
...
function startServer(env: NodeJS.ProcessEnv): ReturnType<typeof spawn> {
  ...
  return spawn({
    cmd: ['bun', 'run', '--filter', '@orion/server', 'start'],
    cwd: MONOREPO_ROOT,
    stdout: 'pipe',
    stderr: 'pipe',
    env,
...
async function main() {
  ...
  if (args.isNew) {
    ...
    USER_DATA_DIR = mkdtempSync(join(tmpdir(), 'orion-dev-'))
