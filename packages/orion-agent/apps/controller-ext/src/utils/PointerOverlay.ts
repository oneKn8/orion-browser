...
export class PointerOverlay {
  private static orion = getOrionAdapter()
  ...
  static async showPointer(
    ...
    const pointerId = `orion-pointer-${Date.now()}`
    ...
    const script = `
      (function() {
        var existing = document.querySelector('orion-pointer');
        if (existing) existing.remove();

        if (!customElements.get('orion-pointer')) {
          customElements.define('orion-pointer', class extends HTMLElement {
            constructor() {
              super();
        ...
        var host = document.createElement('orion-pointer');
        host.id = '${pointerId}';
    ...
    try {
      await PointerOverlay.orion.executeJavaScript(tabId, script)
      logger.debug(
