...
**⚠️ IMPORTANT:** This extension ONLY works in **Orion Chrome**, not regular Chrome!
...
### 2. Load Extension in Orion Chrome
...
6. Verify extension is loaded (you should see "Orion Controller")
...
```
🚀 Starting Orion Controller Test Client
──────────────────────────────────────────────────────────
...
**If you see "available": false**, you're not using Orion Chrome.
...
LOGGING_PREFIX=[Orion Controller]      # Log message prefix (default: [Orion Controller])
```
...
Available Commands:
  ...
  Diagnostic:
  d. checkOrion     - Check if chrome.browserOS is available
...
2. Click **"Inspect views service worker"** under "Orion Controller"
3. Service worker console shows all logs

**Check extension status:**

```javascript
__orionController.getStats();
```
...
```
Registered 14 action(s): checkOrion, getActiveTab, getTabs, ...
```
...
| ------------------------ | --------------------------------- | ------------------------------- | -------------------------------------- |
| `checkOrion`         | `{}`                              | `{available, apis}`             | Check if chrome.browserOS is available |
...
**Cause:** Not using Orion Chrome

**Solution:**

- Download and use Orion Chrome (not regular Chrome)
- Verify at `chrome://version` - should show "Orion" in the name
...
2. ✅ Extension loaded in Orion Chrome
...
- **Orion-agent**: AI agent that uses this controller for browser automation
- **Orion Chrome**: Custom Chrome build with `chrome.browserOS` APIs
...
4. Verify you're using Orion Chrome (run diagnostic test)
