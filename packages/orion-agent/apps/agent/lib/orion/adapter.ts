...
export class OrionAdapter {
  ...
  async getOrionVersion(): Promise<string | null> {
    return new Promise<string | null>((resolve, reject) => {
      if (typeof chrome.browserOS.getOrionVersionNumber !== 'function') {
        resolve(null)
        return
      }

      chrome.browserOS.getOrionVersionNumber((version: string) => {
        if (chrome.runtime.lastError) {
          reject(new Error(chrome.runtime.lastError.message || 'Unknown error'))
        } else {
          resolve(version)
  ...
  async choosePath(options?: ChoosePathOptions): Promise<SelectedPath | null> {
    ...
    return new Promise<SelectedPath | null>((resolve, reject) => {
      ...
      if (options) {
        chrome.orion.choosePath(options, callback)
      } else {
        chrome.orion.choosePath(callback)
      }
    })
  }
