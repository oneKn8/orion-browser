...
import { getOrionAdapter } from './adapter'
import { Capabilities, Feature } from './capabilities'
import { ORION_PREFS } from './prefs'
...
async function getAgentPort(): Promise<number> {
  if (env.VITE_ORION_SERVER_PORT) {
    return env.VITE_ORION_SERVER_PORT
  }

  try {
    const adapter = getOrionAdapter()
    const pref = await adapter.getPref(ORION_PREFS.AGENT_PORT)

    if (pref?.value && typeof pref.value === 'number') {
      return pref.value
    }
  } catch {
    // Orion API not available
  }
...
async function getMcpPort(): Promise<number> {
  try {
    const adapter = getOrionAdapter()
    const pref = await adapter.getPref(ORION_PREFS.MCP_PORT)
...
async function getProxyPort(): Promise<number> {
  try {
    const adapter = getOrionAdapter()
    const pref = await adapter.getPref(ORION_PREFS.PROXY_PORT)
