import { OrionAdapter } from './adapter'

const SERVER_VERSION_PREF = 'orion.server.version'

type FeatureConfig = {
  minOrionVersion?: string
  maxOrionVersion?: string
  minServerVersion?: string
  maxServerVersion?: string
}

/**
 * Features gated by Orion version.
...
export enum Feature {
  ...
  // Workspace folder selection with full path support requires new orion.choosePath API
 ...
 * - minOrionVersion: feature enabled when Orion >= this version
 * - maxOrionVersion: feature enabled when Orion < this version (for deprecation)
...
const FEATURE_CONFIG: { [K in Feature]: FeatureConfig } = {
  [Feature.OPENAI_COMPATIBLE_SUPPORT]: { minOrionVersion: '0.33.0.1' },
  [Feature.MANAGED_MCP_SUPPORT]: { minOrionVersion: '0.34.0.0' },
  [Feature.PERSONALIZATION_SUPPORT]: { minOrionVersion: '0.36.1.0' },
  [Feature.UNIFIED_PORT_SUPPORT]: { minOrionVersion: '0.36.1.0' },
  [Feature.CUSTOMIZATION_SUPPORT]: { minOrionVersion: '0.36.1.0' },
  [Feature.WORKSPACE_FOLDER_SUPPORT]: { minOrionVersion: '0.36.4.0' },
  [Feature.PROXY_SUPPORT]: { minOrionVersion: '0.39.0.1' },
  ...
  [Feature.NEWTAB_CHAT_SUPPORT]: { minOrionVersion: '0.40.0.0' },
  [Feature.VERTICAL_TABS_SUPPORT]: { minOrionVersion: '0.42.0.0' },
  [Feature.MEMORY_SUPPORT]: { minServerVersion: '0.0.73' },
  [Feature.SKILLS_SUPPORT]: { minOrionVersion: '0.43.0.0' },
}
...
async function doInitialize(): Promise<CapabilitiesState> {
  const adapter = OrionAdapter.getInstance()
  ...
  try {
    const versionStr = await adapter.getOrionVersion()
    if (versionStr) {
      state.orionVersion = parseVersion(versionStr)
    }
  } catch {
    // Orion version unknown - features requiring it will be disabled
  }
...
function checkFeatureSupport(
  ...
  const hasOrionConstraints =
    config.minOrionVersion || config.maxOrionVersion
  if (
    hasOrionConstraints &&
    !checkVersionConstraints(
      state.orionVersion,
      config.minOrionVersion,
      config.maxOrionVersion,
    )
  ) {
    return false
  }
...
export const Capabilities = {
  ...
  async getOrionVersion(): Promise<string | null> {
    const state = await ensureInitialized()
    if (!state.orionVersion) return null
    return state.orionVersion.join('.')
  },
