...
var rootCmd = &cobra.Command{
	Use:           "orion-cli",
	Short:         "Browser control CLI for Orion",
	Long:          "orion-cli — command-line interface for controlling Orion via MCP",
	SilenceUsage:  true,
	SilenceErrors: true,
}
...
func init() {
	...
	rootCmd.PersistentFlags().StringVarP(&serverURL, "server", "s", defaultServerURL(), "Orion server URL")
...
func resolvePageID(c *mcp.Client) (int, error) {
	...
	if env := os.Getenv("ORION_PAGE"); env != "" {
...
func defaultServerURL() string {
	if env := normalizeServerURL(os.Getenv("ORION_URL")); env != "" {
		return env
	}
...
func validateServerURL(raw string) (string, error) {
	...
	return "", fmt.Errorf(
		"Orion server URL is not configured.\n  Open Orion -> Settings -> Orion MCP and copy the Server URL.\n  Then run: orion-cli init",
	)
}
