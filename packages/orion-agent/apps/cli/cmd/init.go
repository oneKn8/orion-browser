...
func init() {
	cmd := &cobra.Command{
		Use:   "init",
		Short: "Configure the Orion server connection",
		Long: `Set up the CLI by providing the MCP server URL from Orion.

Open Orion → Settings → Orion MCP to find your Server URL.
...
Run again if your port changes.`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			fmt.Println()
			bold.Println("Orion CLI Setup")
			fmt.Println()
			fmt.Println("Open Orion → Settings → Orion MCP")
			...
			fmt.Println()
			dim.Println("Try: orion-cli health")
			dim.Println("     orion-cli pages")
		},
	}
