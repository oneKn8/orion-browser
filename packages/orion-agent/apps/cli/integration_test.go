...
func TestMain(m *testing.M) {
	...
	tmpDir, err := os.MkdirTemp("", "orion-cli-test-*")
	if err != nil {
		fmt.Fprintf(os.Stderr, "Failed to create temp dir: %v\n", err)
		os.Exit(1)
	}

	cliBinary = filepath.Join(tmpDir, "orion-cli")
...
func TestVersion(t *testing.T) {
	...
	if !strings.Contains(r.Stdout, "orion-cli") {
		t.Errorf("expected version output to contain 'orion-cli', got: %s", r.Stdout)
	}
}
