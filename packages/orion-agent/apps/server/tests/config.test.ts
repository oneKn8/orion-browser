...
describe('loadServerConfig', () => {
  ...
  beforeEach(() => {
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'orion-config-test-'))
    originalEnv = { ...process.env }

    // Clear relevant env vars
    delete process.env.ORION_CDP_PORT
    delete process.env.ORION_SERVER_PORT
    delete process.env.ORION_EXTENSION_PORT
    delete process.env.ORION_RESOURCES_DIR
    delete process.env.ORION_EXECUTION_DIR
    delete process.env.ORION_INSTALL_ID
    delete process.env.ORION_CLIENT_ID
    delete process.env.ORION_AI_SDK_DEVTOOLS
  })
  ...
  describe('environment variables', () => {
    it('reads from env when CLI not provided', () => {
      process.env.ORION_CDP_PORT = '9222'
      process.env.ORION_SERVER_PORT = '9223'
      process.env.ORION_EXTENSION_PORT = '9224'
    ...
    it('CLI takes precedence over env', () => {
      process.env.ORION_SERVER_PORT = '9999'
      process.env.ORION_EXTENSION_PORT = '9999'
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({
          ...
          instance: {
            ...
            orion_version: true, // should be string
          },
        }),
      )
  ...
  describe('AI SDK DevTools', () => {
    it('enables devtools via ORION_AI_SDK_DEVTOOLS env var', () => {
      process.env.ORION_AI_SDK_DEVTOOLS = 'true'
