...
# Cleanup script for Orion test resources
# Kills any running test processes and removes orphaned temp directories

set -e

echo "Cleaning up Orion test resources..."
...
TEMP_DIR="${TMPDIR:-/tmp}"
temp_dirs=$(find "$TEMP_DIR" -maxdepth 1 -name "orion-test-*" -type d 2>/dev/null | wc -l | tr -d ' ')

if [ "$temp_dirs" -gt 0 ]; then
  echo "  Removing $temp_dirs orphaned temp directories"
  find "$TEMP_DIR" -maxdepth 1 -name "orion-test-*" -type d -exec rm -rf {} + 2>/dev/null || true
fi
