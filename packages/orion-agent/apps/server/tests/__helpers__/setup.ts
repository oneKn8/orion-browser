...
async function waitForExtensionConnection(
  port: number,
  // Extension startup can be slow on a cold Orion profile.
...
/**
 * Ensures the full Orion test environment is ready:
 ...
 */
export async function ensureOrion(
  ...
  console.log('\n=== Setting up Orion test environment ===')
...
/**
 * Cleans up the full Orion test environment.
 */
export async function cleanupOrion(): Promise<void> {
  console.log('\n=== Cleaning up Orion test environment ===')
  await killBrowser()
  await killServer()
  runtimePlan = null
