...
function createOrionModel(config: ResolvedLLMConfig): LanguageModel {
  if (!config.baseUrl) throw new Error('Orion provider requires baseUrl')
  ...
  logger.debug('Creating OpenAI-compatible provider for Orion')
  return createOpenAICompatible({
    name: 'orion',
    baseURL: baseUrl,
    ...(apiKey && { apiKey }),
...
const PROVIDER_FACTORIES: Record<string, ProviderFactory> = {
  ...
  [LLM_PROVIDERS.ORION]: createOrionModel,
