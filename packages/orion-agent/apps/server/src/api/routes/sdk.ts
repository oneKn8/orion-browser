 ...
 *
 * SDK Routes - REST API for @oneKn8/agent-sdk
 */
...
export function createSdkRoutes(deps: SdkDeps) {
  const { port, browser, orionId } = deps
  ...
  return new Hono<Env>()
    ...
    .post('/act', zValidator('json', ActRequestSchema), async (c) => {
      ...
      const llmConfig = llm ?? { provider: LLM_PROVIDERS.ORION }

      if (llmConfig.provider !== LLM_PROVIDERS.ORION && !llmConfig.model) {
        return c.json(
          {
            error: { message: 'model is required for non-orion providers' },
          },
          400,
        )
      }
