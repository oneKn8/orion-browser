...
import { EXIT_CODES } from '@orion/shared/constants/exit-codes'
...
import { cleanOldSessions, ensureBrowserosDir } from './lib/orion-dir'
...
export class Application {
  ...
  async start(): Promise<void> {
    logger.info(`Starting Orion Server v${VERSION}`)
    ...
    const dailyRateLimit = await fetchDailyRateLimit(identity.getOrionId())
  ...
  private async initCoreServices(): Promise<void> {
    ...
    const orionId = identity.getOrionId()
    logger.info('Orion ID initialized', {
    ...
    metrics.initialize({
      ...
      orion_version: this.config.instanceBrowserosVersion,
