...
function parseCliArgs(argv: string[]): ConfigResult<ParsedCliArgs> {
  ...
  try {
    program
      .name('orion-server')
      .description('Orion Unified Server - MCP + Agent')
      .version(VERSION)
    ...
    return {
      ...
      value: omitUndefined({
        ...
        instanceBrowserosVersion:
          typeof cfg.instance?.orion_version === 'string'
            ? cfg.instance.orion_version
            : undefined,
...
function parseRuntimeEnv(): PartialConfig {
  ...
  return omitUndefined({
    cdpPort: process.env.ORION_CDP_PORT
      ? safeParseInt(process.env.ORION_CDP_PORT)
      : undefined,
    serverPort: process.env.ORION_SERVER_PORT
      ? safeParseInt(process.env.ORION_SERVER_PORT)
      : undefined,
    extensionPort: process.env.ORION_EXTENSION_PORT
      ? safeParseInt(process.env.ORION_EXTENSION_PORT)
      : undefined,
    resourcesDir: process.env.ORION_RESOURCES_DIR
      ? toAbsolutePath(process.env.ORION_RESOURCES_DIR, cwd)
      : undefined,
    executionDir: process.env.ORION_EXECUTION_DIR
      ? toAbsolutePath(process.env.ORION_EXECUTION_DIR, cwd)
      : undefined,
    instanceInstallId: process.env.ORION_INSTALL_ID,
    instanceClientId: process.env.ORION_CLIENT_ID,
    aiSdkDevtoolsEnabled:
      process.env.ORION_AI_SDK_DEVTOOLS === 'true' ? true : undefined,
  })
}
