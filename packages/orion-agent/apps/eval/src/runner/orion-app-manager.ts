 ...
 * Orion App Manager
 *
 * Manages Orion lifecycle for eval workers.
...
export class OrionAppManager {
  ...
  static buildExtensions(): void {
    console.log(`[ORION] Building controller extension...`)
    ...
    console.log(`[ORION] Controller extension built`)
  }
  ...
  async restart(): Promise<void> {
    ...
    throw new Error(
      `Failed to start Orion after ${MAX_RESTART_ATTEMPTS} attempts`,
    )
  }
   ...
   *   --disable-orion-server  (we run our own server)
   *   --disable-orion-extensions  (we load them explicitly if needed)
   *   --remote-debugging-port, --orion-mcp-port, --orion-extension-port
  ...
  private async startAll(): Promise<void> {
    ...
    this.tempDir = mkdtempSync('/tmp/orion-eval-')
    ...
    const chromeArgs = [
      ...
      `--orion-mcp-port=${server}`,
      `--orion-extension-port=${extension}`,
    ...
    const serverEnv = {
      ...
      ORION_CDP_PORT: String(cdp),
      ORION_SERVER_PORT: String(server),
      ORION_EXTENSION_PORT: String(extension),
      VITE_ORION_SERVER_PORT: String(server),
    }
