 ...
 * Parallel Executor
 *
 * Each worker gets its own isolated Orion stack:
 *   - OrionAppManager (Chrome + Server on unique ports)
...
import { OrionAppManager } from './orion-app-manager'
...
export class ParallelExecutor {
  ...
  private readonly appManagers = new Map<number, OrionAppManager>()
  ...
  async execute(
    ...
    const loadExtensions = this.config.config.orion.load_extensions ?? false
    if (loadExtensions) {
      OrionAppManager.buildExtensions()
    }
  ...
  private async runWorker(
    ...
    const basePorts: EvalPorts = {
      cdp: this.config.config.orion.base_cdp_port,
      server: this.config.config.orion.base_server_port,
      extension: this.config.config.orion.base_extension_port,
    }
    const headless = this.config.config.orion.headless ?? false
    const appManager = new OrionAppManager(
      workerIndex,
      basePorts,
      loadExtensions,
      headless,
    ...
    try {
      ...
      console.log(`\n  Worker ${workerIndex}: Starting Orion stack...`)
