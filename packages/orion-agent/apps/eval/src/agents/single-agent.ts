...
import { AiSdkAgent } from '@orion/server/agent/tool-loop'
import type { ResolvedAgentConfig } from '@orion/server/agent/types'
import { Browser } from '@orion/server/browser'
import { CdpBackend } from '@orion/server/browser/backends/cdp'
import { registry } from '@orion/server/tools/registry'
...
function extractCdpPort(config: EvalConfig): number {
  const serverUrl = config.orion.server_url
  const match = serverUrl.match(/:(\d+)$/)
  if (!match) return config.orion.base_cdp_port
  const serverPort = Number.parseInt(match[1], 10)
  const workerOffset = serverPort - config.orion.base_server_port
  return config.orion.base_cdp_port + workerOffset
}
...
export class SingleAgentEvaluator implements AgentEvaluator {
  ...
  async execute(): Promise<AgentResult> {
    ...
    const agentConfig: ResolvedAgentConfig = {
      ...
      workingDir: `/tmp/orion-eval-${conversationId}`,
      evalMode: true,
      supportsImages,
    }
