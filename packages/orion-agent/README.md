...
Monorepo for the Orion-agent -- contains 3 packages: agent-UI, server (which contains the agent loop) and controller-extension (which is used by the tools within the agent loop).

> **⚠️ NOTE:** This is only a submodule, the main project is at -- https://github.com/oneKn8/orion-browser
...
apps/
  ...
  controller-ext/  # Orion Controller (Chrome extension for chrome.* APIs)
...
| `apps/controller-ext` | Orion Controller - Chrome extension that bridges `chrome.*` APIs (tabs, bookmarks, history) to the server via WebSocket |
...
- `apps/controller-ext`: Orion Controller - a Chrome extension that bridges `chrome.*` APIs to the server. Controller tools within the server communicate with this extension via WebSocket.
...
└──────────────────────────────────────────────────────────────────────────┘
          ...
          ▼                                         ▼
┌─────────────────────┐              ┌─────────────────────────────────────┐
│   Chromium CDP      │              │   Orion Controller Extension    │
...
|------|--------------|---------|
| 9100 | `ORION_SERVER_PORT` | HTTP server - MCP endpoints, agent chat, health |
| 9000 | `ORION_CDP_PORT` | Chromium CDP server (Orion Server connects as client) |
| 9300 | `ORION_EXTENSION_PORT` | WebSocket server for controller extension |
...
|----------|---------|-------------|
| `ORION_SERVER_PORT` | 9100 | HTTP server port (MCP, chat, health) |
| `ORION_CDP_PORT` | 9000 | Chromium CDP port (server connects as client) |
| `ORION_EXTENSION_PORT` | 9300 | WebSocket port for controller extension |
| `ORION_CONFIG_URL` | - | Remote config endpoint for rate limits |
| `ORION_INSTALL_ID` | - | Unique installation identifier (analytics) |
| `ORION_CLIENT_ID` | - | Client identifier (analytics) |
| `POSTHOG_API_KEY` | - | Server-side PostHog API key |
| `SENTRY_DSN` | - | Server-side Sentry DSN |
| `ORION_TEST_HEADLESS` | false | Headless mode for server tests |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `ORION_CONFIG_URL` | - | Remote config endpoint baked into prod binary |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `ORION_SERVER_PORT` | 9100 | Passed to Orion via CLI args |
| `ORION_CDP_PORT` | 9000 | Passed to Orion via CLI args |
| `ORION_EXTENSION_PORT` | 9300 | Passed to Orion via CLI args |
| `VITE_ORION_SERVER_PORT` | 9100 | Agent UI connects to server (must match `ORION_SERVER_PORT`) |
| `ORION_BINARY` | - | Path to Orion binary |
| `USE_ORION_BINARY` | true | Use Orion instead of default Chrome |
