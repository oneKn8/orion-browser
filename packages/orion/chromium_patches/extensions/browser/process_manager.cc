...
+  // Orion: Add permanent keepalive for Orion extensions to prevent
+  // their service workers from being terminated due to inactivity.
+  if (orion::IsOrionExtension(worker_id.extension_id)) {
+    base::Uuid keepalive_uuid = IncrementServiceWorkerKeepaliveCount(
+        worker_id,
+        content::ServiceWorkerExternalRequestTimeoutType::kDoesNotTimeout,
+        Activity::PROCESS_MANAGER,
+        "orion_permanent_keepalive");
+    orion_permanent_keepalives_[worker_id] = keepalive_uuid;
+    VLOG(1) << "orion: Added permanent keepalive for extension "
...
+  // Orion: Clean up permanent keepalive for Orion extensions.
+  auto keepalive_iter = orion_permanent_keepalives_.find(worker_id);
+  if (keepalive_iter != orion_permanent_keepalives_.end()) {
+    DecrementServiceWorkerKeepaliveCount(
+        worker_id, keepalive_iter->second, Activity::PROCESS_MANAGER,
+        "orion_permanent_keepalive");
+    orion_permanent_keepalives_.erase(keepalive_iter);
+    VLOG(1) << "orion: Removed permanent keepalive for extension "
+            << worker_id.extension_id;
+  }
+
