diff --git a/chrome/utility/importer/orion/chrome_extensions_importer.cc b/chrome/utility/importer/orion/chrome_extensions_importer.cc
new file mode 100644
index 0000000000000..521ceadec9fc9
--- /dev/null
+++ b/chrome/utility/importer/orion/chrome_extensions_importer.cc
@@ -0,0 +1,98 @@
...
+    LOG(WARNING) << "orion: Failed to read "
...
+  if (!preferences) {
+    LOG(WARNING) << "orion: Failed to parse JSON from "
+                 << preferences_path.BaseName().value();
+    return extension_ids;
+  }
+
...
+    LOG(WARNING) << "orion: No preferences files found";
+    return extension_ids;
+  }
+
