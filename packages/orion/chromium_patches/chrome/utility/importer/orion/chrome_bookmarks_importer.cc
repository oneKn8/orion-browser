diff --git a/chrome/utility/importer/orion/chrome_bookmarks_importer.cc b/chrome/utility/importer/orion/chrome_bookmarks_importer.cc
new file mode 100644
index 0000000000000..e631448aa5137
--- /dev/null
+++ b/chrome/utility/importer/orion/chrome_bookmarks_importer.cc
@@ -0,0 +1,248 @@
...
+#include "chrome/utility/importer/orion/chrome_importer_utils.h"
...
+    LOG(WARNING) << "orion: Bookmarks file not found";
+    return result;
+  }
+
+  std::string bookmarks_content;
+  if (!base::ReadFileToString(bookmarks_path, &bookmarks_content)) {
+    LOG(WARNING) << "orion: Failed to read Bookmarks file";
+    return result;
+  }
+
...
+    LOG(WARNING) << "orion: Failed to parse Bookmarks JSON";
+    return result;
+  }
+
+  const base::DictValue* roots = bookmarks_value->GetDict().FindDict("roots");
+  if (!roots) {
+    LOG(WARNING) << "orion: No roots in Bookmarks";
+    return result;
+  }
+
+  // Import bookmark bar
