diff --git a/chrome/utility/importer/orion/chrome_password_importer.cc b/chrome/utility/importer/orion/chrome_password_importer.cc
new file mode 100644
index 0000000000000..1a01e3951aa3f
--- /dev/null
+++ b/chrome/utility/importer/orion/chrome_password_importer.cc
@@ -0,0 +1,150 @@
...
+    LOG(WARNING) << "orion: Failed to create temp file";
+    return base::FilePath();
+  }
+
+  if (!base::CopyFile(db_path, temp_path)) {
+    LOG(WARNING) << "orion: Failed to copy database to temp";
...
+    LOG(WARNING) << "orion: Failed to extract encryption key, "
...
+    LOG(WARNING) << "orion: Login Data not found at: "
...
+    LOG(WARNING) << "orion: Failed to open Login Data database";
...
+      LOG(WARNING) << "orion: Failed to prepare query";
+      base::DeleteFile(temp_db_path);
+      return passwords;
+    }
+
...
+                                &decrypted_password)) {
+          LOG(WARNING) << "orion: Failed to decrypt password for: "
+                       << origin_url;
+          continue;
+        }
+      }
...
+  base::DeleteFile(temp_db_path);
+
+  LOG(INFO) << "orion: Imported " << passwords.size()
+            << " passwords";
+
+  return passwords;
+}
