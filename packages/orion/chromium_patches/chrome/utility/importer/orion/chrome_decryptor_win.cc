diff --git a/chrome/utility/importer/orion/chrome_decryptor_win.cc b/chrome/utility/importer/orion/chrome_decryptor_win.cc
new file mode 100644
index 0000000000000..d4653cb2abf43
--- /dev/null
+++ b/chrome/utility/importer/orion/chrome_decryptor_win.cc
@@ -0,0 +1,245 @@
+// Copyright 2024 AKW Technology Inc
+// Chrome decryption - Windows implementation
+// Uses DPAPI for key retrieval, AES-256-GCM for decryption
+
+#include "chrome/utility/importer/orion/chrome_decryptor.h"
+
...
+    LOG(WARNING) << "orion: Failed to read Local State file";
+    return false;
+  }
+
+  auto parsed = base::JSONReader::Read(json_content, base::JSON_PARSE_RFC);
+  if (!parsed || !parsed->is_dict()) {
+    LOG(WARNING) << "orion: Failed to parse Local State JSON";
+    return false;
+  }
+
...
+  if (!os_crypt) {
+    LOG(WARNING) << "orion: No os_crypt section in Local State";
+    return false;
+  }
+
+  const std::string* encoded_key = os_crypt->FindString("encrypted_key");
+  if (!encoded_key || encoded_key->empty()) {
+    LOG(WARNING) << "orion: No encrypted_key in os_crypt";
+    return false;
+  }
+
...
+  if (!decoded) {
+    LOG(WARNING) << "orion: Failed to base64 decode encrypted_key";
+    return false;
+  }
+
...
+    LOG(WARNING) << "orion: Encrypted data too short";
+    return false;
+  }
+
+  // Check for "DPAPI" prefix
+  if (!base::StartsWith(encrypted_data, kDpapiPrefix)) {
+    LOG(WARNING) << "orion: Missing DPAPI prefix";
+    return false;
+  }
+
+  // Strip "DPAPI" prefix
...
+    LOG(WARNING) << "orion: CryptUnprotectData failed with error: "
+                 << error;
+    return false;
+  }
+
...
+
+  if (!decrypted) {
+    LOG(WARNING) << "orion: AES-GCM decryption failed";
+    return false;
+  }
+
...
+    LOG(WARNING) << "orion: Local State file not found at: "
...
+    LOG(WARNING) << "orion: Unexpected key length: "
...
+    LOG(INFO) << "orion: Value doesn't have v10 prefix, may be unencrypted";
...
+    LOG(WARNING) << "orion: Empty ciphertext after prefix";
+    return false;
+  }
+
