diff --git a/chrome/utility/importer/orion/chrome_decryptor_mac.mm b/chrome/utility/importer/orion/chrome_decryptor_mac.mm
new file mode 100644
index 0000000000000..caabfa17010ab
--- /dev/null
+++ b/chrome/utility/importer/orion/chrome_decryptor_mac.mm
@@ -0,0 +1,191 @@
...
+      LOG(WARNING) << "orion: Chrome Safe Storage not found in Keychain";
+    } else if (error == errSecAuthFailed) {
+      LOG(WARNING) << "orion: Keychain access denied";
+    } else {
+      LOG(WARNING) << "orion: Keychain error: " << error;
+    }
+    return false;
+  }
+
...
+    LOG(WARNING) << "orion: Invalid key size";
+    return false;
+  }
+
...
+                          key.data(), kIv)) {
+    LOG(WARNING) << "orion: EVP_DecryptInit_ex failed";
+    return false;
+  }
+
...
+    LOG(WARNING) << "orion: EVP_DecryptUpdate failed";
+    return false;
+  }
+
+  int final_length = 0;
...
+                           &final_length)) {
+    LOG(WARNING) << "orion: EVP_DecryptFinal_ex failed - possible padding error";
+    return false;
+  }
+
...
+    LOG(WARNING) << "orion: PBKDF2 key derivation failed";
+    if (result) {
