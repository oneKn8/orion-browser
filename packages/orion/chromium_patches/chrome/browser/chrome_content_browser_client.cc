...
+// Handles chrome://orion/* URLs by rewriting to extension URLs.
+// Forward handler: chrome://orion/ai -> chrome-extension://[id]/options.html
+static bool HandleOrionURL(GURL* url,
+                               content::BrowserContext* browser_context) {
+  if (!url->SchemeIs(content::kChromeUIScheme) ||
+      url->host() != orion::kOrionHost) {
+    return false;
+  }
+
+  std::string extension_url =
+      orion::GetOrionExtensionURL(url->path());
...
+}
+
+// Reverse handler: chrome-extension://[id]/options.html#ai -> chrome://orion/ai
+// This ensures the virtual URL is shown in the address bar.
+static bool ReverseOrionURL(GURL* url,
+                                content::BrowserContext* browser_context) {
+  if (!url->SchemeIs(extensions::kExtensionScheme)) {
+    return false;
+  }
+
+  std::string virtual_url = orion::GetOrionVirtualURL(
...
+  // Handler to rewrite chrome://orion/* to extension URLs.
+  handler->AddHandlerPair(&HandleOrionURL, &ReverseOrionURL);
...
+#if BUILDFLAG(ENABLE_EXTENSIONS_CORE)
+  // Allow Orion extensions to access private networks (e.g., localhost).
+  // This enables extension service workers to connect to local servers.
+  if (origin.scheme() == extensions::kExtensionScheme &&
+      orion::IsOrionExtension(origin.host())) {
