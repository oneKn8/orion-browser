diff --git a/chrome/browser/resources/settings/orion_prefs_page/orion_prefs_page.ts b/chrome/browser/resources/settings/orion_prefs_page/orion_prefs_page.ts
new file mode 100644
index 0000000000000..463bbed53b5b3
--- /dev/null
+++ b/chrome/browser/resources/settings/orion_prefs_page/orion_prefs_page.ts
@@ -0,0 +1,249 @@
...
+
+/**
+ * @fileoverview 'settings-orion-prefs-page' contains Orion-specific settings.
+ */
+
...
+
+import {getTemplate} from './orion_prefs_page.html.js';
+
...
+}
+
+export interface SettingsOrionPrefsPageElement {
+  $: {
+    addProviderDialog: HTMLElement;
+  };
+}
+
+const SettingsOrionPrefsPageElementBase = PrefsMixin(PolymerElement);
+
+export class SettingsOrionPrefsPageElement extends SettingsOrionPrefsPageElementBase {
+  static get is() {
+    return 'settings-orion-prefs-page';
+  }
+
+  static get template() {
+    return getTemplate();
...
+    try {
+      const pref = this.getPref('orion.custom_providers');
+      if (pref && pref.value) {
...
+    this.setPrefValue('orion.custom_providers', customProvidersJson);
+  }
+  
+  /**
...
+    'settings-orion-prefs-page': SettingsOrionPrefsPageElement;
+  }
+}
+
+customElements.define(
+    SettingsOrionPrefsPageElement.is, SettingsOrionPrefsPageElement);
