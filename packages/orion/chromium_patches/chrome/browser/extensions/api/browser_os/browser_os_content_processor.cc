...
+    LOG(INFO) << "orion: ExtractPageContent - tree is empty";
+    return items;
+  }
+
+  LOG(INFO) << "orion: ExtractPageContent - processing " << tree_update.nodes.size() << " nodes";
+
...
+
+  LOG(INFO) << "orion: ExtractPageContent - extracted " << items.size() << " items";
+
+  return items;
+}
+
