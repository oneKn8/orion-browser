...
+            &is_offscreen);
+        
+        VLOG(3) << "[orion] Node " << node_data.id 
...
+    } else {
+      // No AXTree available
+      LOG(WARNING) << "[orion] No AXTree available for bounds computation";
+    }
+    
...
+  }
+  
+  LOG(INFO) << "[orion] Viewport: " << viewport_size.ToString() 
...
+  
+  if (!ax_tree) {
+    LOG(ERROR) << "[orion] Failed to create AXTree from update";
+    SnapshotProcessingResult result;
...
+  }
+  
+  LOG(INFO) << "[orion] Created AXTree with " << tree_update.nodes.size() 
