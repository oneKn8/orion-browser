...
+  content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
+  if (!rfh) {
+    LOG(WARNING) << "[orion] No RenderFrameHost for AccessibilityDoDefault";
+    return false;
+  }
+  
...
+  
+  LOG(INFO) << "[orion] Performing AccessibilityDoDefault on node " 
...
+  content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
+  if (!rfh) {
+    LOG(WARNING) << "[orion] No RenderFrameHost for AccessibilityFocus";
+    return false;
+  }
+  
+  ui::AXActionData action_data;
...
+  action_data.target_tree_id = node_info.ax_tree_id;
+  
+  LOG(INFO) << "[orion] Performing AccessibilityFocus on node " 
+            << node_info.ax_node_id;
+  
+  rfh->AccessibilityPerformAction(action_data);
+  return true;
...
+  content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
+  if (!rfh) {
+    LOG(WARNING) << "[orion] No RenderFrameHost for AccessibilityScrollToMakeVisible";
+    return false;
+  }
+  
+  ui::AXActionData action_data;
...
+  
+  LOG(INFO) << "[orion] Performing AccessibilityScrollToMakeVisible on node " 
+            << node_info.ax_node_id;
+  
+  rfh->AccessibilityPerformAction(action_data);
+  return true;
...
+    LOG(INFO) << "[orion] Node is out of viewport, scrolling to make visible";
...
+    // LOG(INFO) << "[orion] Node was out of viewport, trying AccessibilityDoDefault click first";
+    // bool changed = OrionChangeDetector::ExecuteWithDetection(
+    //     web_contents,
...
+      LOG(INFO) << "[orion] No change from accessibility click, trying HTML click";
+      changed = OrionChangeDetector::ExecuteWithDetection(
+          web_contents,
+          [&]() { HtmlClick(web_contents, node_info); },
+          base::Milliseconds(200));
+    }
+    
+    LOG(INFO) << "[orion] Click result: " << (changed ? "changed" : "no change");
+    return changed;
+  }
+  
+  // For in-viewport nodes, try coordinate click first (most natural)
+  LOG(INFO) << "[orion] Node is in viewport, trying coordinate click first";
+  gfx::PointF click_point = GetNodeCenterPoint(web_contents, node_info);
+  
+  bool changed = OrionChangeDetector::ExecuteWithDetection(
+      web_contents,
...
+  content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
+  if (!rfh) {
+    LOG(WARNING) << "[orion] No RenderFrameHost for AccessibilitySetValue";
+    return false;
+  }
+  
+  ui::AXActionData action_data;
...
+  
+  LOG(INFO) << "[orion] Performing AccessibilitySetValue on node " 
...
+  
+  if (is_out_of_viewport) {
+    LOG(INFO) << "[orion] Node is out of viewport for typing, scrolling to make visible";
+    AccessibilityScrollToMakeVisible(web_contents, node_info, true /* center */);
+    // Wait for scroll to complete
+    base::PlatformThread::Sleep(base::Milliseconds(300));
+  }
...
+  if (!changed) {
+    LOG(INFO) << "[orion] No change from native typing, trying JavaScript";
+    changed = OrionChangeDetector::ExecuteWithDetection(
+        web_contents,
...
+  // if (!changed) {
+  //   LOG(INFO) << "[orion] No change from JavaScript, trying accessibility SetValue";
+  //   changed = OrionChangeDetector::ExecuteWithDetection(
+  //       web_contents,
+  //       [&]() {
...
+  // }
+  
+  LOG(INFO) << "[orion] Type result: " << (changed ? "changed" : "no change");
+  return changed;
+}
+
...
+      base::Milliseconds(200));
+  
+  LOG(INFO) << "[orion] Clear result: " << (changed ? "changed" : "no change");
+  return changed;
+}
+
...
+      base::Milliseconds(200));
+  
+  LOG(INFO) << "[orion] KeyPress result for '" << key << "': " 
...
+    LOG(INFO) << "[orion] No interactive elements in viewport to highlight";
+    return;
+  }
+  
+  LOG(INFO) << "[orion] Highlighting " << filtered_nodes.size() 
...
+      document.querySelectorAll('.orion-bbox').forEach(e => e.remove());
+      
+      // Create container for all bounding boxes
+      const container = document.createElement('div');
+      container.className = 'orion-bbox-container';
...
+      document.querySelectorAll('.orion-bbox-container').forEach(e => e.remove());
+      document.querySelectorAll('.orion-bbox').forEach(e => e.remove());
+      
+      // Remove all highlight containers  
+      document.querySelectorAll('.orion-highlight-container').forEach(e => e.remove());
+      document.querySelectorAll('.orion-highlight').forEach(e => e.remove());
+      
+      // Remove any style elements we added
+      document.querySelectorAll('#orion-highlight-styles').forEach(e => e.remove());
+      
+      return true;
+    })();
+  )";
...
+                                   const gfx::PointF& point) {
+  LOG(INFO) << "[orion] ClickCoordinatesWithDetection at (" 
+            << point.x() << ", " << point.y() << ")";
+  
...
+      base::Milliseconds(300));
+  
+  LOG(INFO) << "[orion] Click coordinates result: " 
+            << (changed ? "changed" : "no change");
+  return changed;
+}
+
...
+                                    const gfx::PointF& point,
+                                    const std::string& text) {
+  LOG(INFO) << "[orion] TypeAtCoordinatesWithDetection at (" 
...
+  if (!changed) {
+    LOG(INFO) << "[orion] No change from native typing at coordinates, trying JS injection";
+    
...
+  }
+  
+  LOG(INFO) << "[orion] Type at coordinates result: " 
