...
+uint32_t OrionGetInteractiveSnapshotFunction::next_snapshot_id_ = 1;
+
+// Constructor and destructor implementations
+OrionGetInteractiveSnapshotFunction::OrionGetInteractiveSnapshotFunction() = default;
+OrionGetInteractiveSnapshotFunction::~OrionGetInteractiveSnapshotFunction() = default;
+
+ExtensionFunction::ResponseAction OrionGetAccessibilityTreeFunction::Run() {
...
+      base::BindOnce(
+          &OrionGetAccessibilityTreeFunction::OnAccessibilityTreeReceived,
+          this),
...
+}
+
+void OrionGetAccessibilityTreeFunction::OnAccessibilityTreeReceived(
...
+}
+
+// Implementation of OrionGetInteractiveSnapshotFunction
+
+ExtensionFunction::ResponseAction OrionGetInteractiveSnapshotFunction::Run() {
...
+    LOG(WARNING) << "[orion] Frame not stable for AX snapshot - skipping";
...
+  web_contents->RequestAXTreeSnapshot(
+      base::BindOnce(
+          &OrionGetInteractiveSnapshotFunction::OnAccessibilityTreeReceived,
+          this),
+      ui::AXMode(ui::AXMode::kWebContents | ui::AXMode::kExtendedProperties |
+                 ui::AXMode::kInlineTextBoxes),
+      /* max_nodes= */ 0,  // No limit
...
+}
+
+void OrionGetInteractiveSnapshotFunction::OnAccessibilityTreeReceived(
+    ui::AXTreeUpdate& tree_update) {
+  // Double-check frame is still valid before processing
+  if (!web_contents_) {
+    LOG(WARNING) << "[orion] WebContents gone during AX snapshot callback";
+    browser_os::InteractiveSnapshot empty_snapshot;
+    empty_snapshot.snapshot_id = next_snapshot_id_++;
+    empty_snapshot.timestamp = base::Time::Now().InMillisecondsFSinceUnixEpoch();
+    empty_snapshot.processing_time_ms = 0;
...
+    LOG(WARNING) << "[orion] Frame became unstable during AX snapshot callback";
+    browser_os::InteractiveSnapshot empty_snapshot;
+    empty_snapshot.snapshot_id = next_snapshot_id_++;
+    empty_snapshot.timestamp = base::Time::Now().InMillisecondsFSinceUnixEpoch();
+    empty_snapshot.processing_time_ms = 0;
...
+      web_contents_.get(),
+      base::BindOnce(
+          &OrionGetInteractiveSnapshotFunction::OnSnapshotProcessed,
+          base::WrapRefCounted(this)));
+}
+
+void OrionGetInteractiveSnapshotFunction::OnSnapshotProcessed(
+    SnapshotProcessingResult result) {
+  Respond(ArgumentList(
+      browser_os::GetInteractiveSnapshot::Results::Create(result.snapshot)));
+}
+
+// Implementation of OrionClickFunction
+
+ExtensionFunction::ResponseAction OrionClickFunction::Run() {
...
+}
+
+// Implementation of OrionInputTextFunction
+
+ExtensionFunction::ResponseAction OrionInputTextFunction::Run() {
...
+  const NodeInfo& node_info = node_it->second;
+  
+  LOG(INFO) << "[orion] InputText: Starting input for nodeId: " << params->node_id;
+  
...
+  
+  if (!change_detected) {
+    LOG(WARNING) << "[orion] InputText: No change detected after typing";
+  }
+  
+  // Create interaction response
+  browser_os::InteractionResponse response;
...
+}
+
+// Implementation of OrionClearFunction
+
+ExtensionFunction::ResponseAction OrionClearFunction::Run() {
...
+  const NodeInfo& node_info = node_it->second;
+  
+  LOG(INFO) << "[orion] Clear: Clearing field for nodeId: " << params->node_id;
+  
+  // Use ClearWithDetection which handles focus and clearing
+  bool change_detected = ClearWithDetection(web_contents, node_info);
+  
+  if (!change_detected) {
+    LOG(WARNING) << "[orion] Clear: No change detected after clearing";
+  }
+  
+  // Create interaction response
+  browser_os::InteractionResponse response;
...
+}
+
+// Implementation of OrionGetPageLoadStatusFunction
+
+ExtensionFunction::ResponseAction OrionGetPageLoadStatusFunction::Run() {
...
+}
+
+// Implementation of OrionScrollUpFunction
+
+ExtensionFunction::ResponseAction OrionScrollUpFunction::Run() {
...
+}
+
+// Implementation of OrionScrollDownFunction
+
+ExtensionFunction::ResponseAction OrionScrollDownFunction::Run() {
...
+}
+
+// Implementation of OrionScrollToNodeFunction
+
+ExtensionFunction::ResponseAction OrionScrollToNodeFunction::Run() {
...
+}
+
+// Implementation of OrionSendKeysFunction
+
+ExtensionFunction::ResponseAction OrionSendKeysFunction::Run() {
...
+  }
+  
+  LOG(INFO) << "[orion] SendKeys: Sending key '" << params->key << "'";
+  
+  // Send the key with change detection
+  bool change_detected = KeyPressWithDetection(web_contents, params->key);
+  
+  if (!change_detected) {
+    LOG(WARNING) << "[orion] SendKeys: No change detected after key press";
+  }
+  
+  // Create interaction response
+  browser_os::InteractionResponse response;
...
+}
+
+// Implementation of OrionCaptureScreenshotFunction
+
+OrionCaptureScreenshotFunction::OrionCaptureScreenshotFunction() = default;
+OrionCaptureScreenshotFunction::~OrionCaptureScreenshotFunction() = default;
+
+ExtensionFunction::ResponseAction OrionCaptureScreenshotFunction::Run() {
...
+    LOG(INFO) << "[orion] CaptureScreenshot: Using exact dimensions: "
...
+      LOG(INFO) << "[orion] CaptureScreenshot: Using thumbnail size: " << max_dimension 
...
+      LOG(INFO) << "[orion] CaptureScreenshot: Using viewport size: " << max_dimension;
+    }
+    
...
+}
+
+void OrionCaptureScreenshotFunction::DrawHighlightsAndCapture() {
...
+      LOG(INFO) << "[orion] Drawing highlights for screenshot with " 
+                << tab_it->second.size() << " interactive elements";
+      if (web_contents_) {
+        ShowHighlights(web_contents_.get(), tab_it->second, true /* show_labels */);
+      }
+    } else {
+      LOG(INFO) << "[orion] No snapshot data available for highlighting";
+    }
+    
...
+        FROM_HERE,
+        base::BindOnce(&OrionCaptureScreenshotFunction::CaptureScreenshotNow,
+                       base::WrapRefCounted(this)),
...
+}
+
+void OrionCaptureScreenshotFunction::CaptureScreenshotNow() {
...
+      base::BindOnce(&OrionCaptureScreenshotFunction::OnScreenshotCaptured,
+                     this));
+}
+
+void OrionCaptureScreenshotFunction::OnScreenshotCaptured(
...
+}
+
+// OrionGetSnapshotFunction implementation
+ExtensionFunction::ResponseAction OrionGetSnapshotFunction::Run() {
...
+  // Request accessibility tree snapshot
+  web_contents->RequestAXTreeSnapshot(
+      base::BindOnce(&OrionGetSnapshotFunction::OnAccessibilityTreeReceived,
+                     this),
+      ui::AXMode(ui::AXMode::kWebContents | ui::AXMode::kExtendedProperties),
+      /* max_nodes= */ 0,  // No limit
+      /* timeout= */ base::TimeDelta(),
...
+}
+
+void OrionGetSnapshotFunction::OnAccessibilityTreeReceived(
+    ui::AXTreeUpdate& tree_update) {
+  if (!has_callback()) {
+    return;
+  }
...
+}
+
+// OrionGetPrefFunction
+ExtensionFunction::ResponseAction OrionGetPrefFunction::Run() {
...
+}
+
+// OrionSetPrefFunction
+ExtensionFunction::ResponseAction OrionSetPrefFunction::Run() {
+  std::optional<browser_os::SetPref::Params> params =
+      browser_os::SetPref::Params::Create(args());
+  EXTENSION_FUNCTION_VALIDATE(params);
+
+  // Security: only allow modifying orion.* prefs
+  if (!params->name.starts_with("orion.")) {
+    return RespondNow(Error("Only orion.* preferences can be modified"));
+  }
+
+  Profile* profile = Profile::FromBrowserContext(browser_context());
+  PrefService* prefs = FindPrefService(params->name, profile);
...
+}
+
+// OrionGetAllPrefsFunction
+ExtensionFunction::ResponseAction OrionGetAllPrefsFunction::Run() {
+  Profile* profile = Profile::FromBrowserContext(browser_context());
+  PrefService* profile_prefs = profile->GetPrefs();
+  PrefService* local_state = g_browser_process->local_state();
+
+  // Build a combined orion prefs dict from both sources
+  base::DictValue combined_orion;
+
+  // Lambda to merge orion prefs from a PrefService
...
+
+    // Look for "orion" key in the top-level dict
+    const base::Value* orion_value = pref_dict.Find("orion");
+    if (orion_value && orion_value->is_dict()) {
+      // Merge this orion dict into combined
+      combined_orion.Merge(orion_value->GetDict().Clone());
+      LOG(INFO) << "[orion] GetAllPrefs: Found orion.* prefs in " << source_name;
+    }
+  };
+
...
+
+  // Create single PrefObject with the entire orion dict
+  std::vector<browser_os::PrefObject> pref_objects;
+  browser_os::PrefObject pref_obj;
+  pref_obj.key = "orion";
+  pref_obj.type = "dictionary";
+  pref_obj.value = base::Value(std::move(combined_orion));
...
+}
+
+// OrionLogMetricFunction
+ExtensionFunction::ResponseAction OrionLogMetricFunction::Run() {
...
+    
+    orion_metrics::OrionMetrics::Log(prefixed_event, std::move(properties));
+  } else {
+    // No properties, just log with extension ID
+    orion_metrics::OrionMetrics::Log(prefixed_event, {
...
+}
+
+// OrionGetVersionNumberFunction
+ExtensionFunction::ResponseAction OrionGetVersionNumberFunction::Run() {
...
+}
+
+// OrionGetBrowserosVersionNumberFunction
+ExtensionFunction::ResponseAction OrionGetBrowserosVersionNumberFunction::Run() {
+  std::string version = std::string(version_info::GetOrionVersionNumber());
+
+  return RespondNow(ArgumentList(
+      browser_os::GetBrowserosVersionNumber::Results::Create(version)));
+}
+
+// OrionExecuteJavaScriptFunction
+ExtensionFunction::ResponseAction OrionExecuteJavaScriptFunction::Run() {
...
+  }
+  
+  LOG(INFO) << "[orion] ExecuteJavaScript: Executing code in tab " << tab_info->tab_id;
+  
...
+      js_code,
+      base::BindOnce(&OrionExecuteJavaScriptFunction::OnJavaScriptExecuted,
+                     this),
...
+}
+
+void OrionExecuteJavaScriptFunction::OnJavaScriptExecuted(base::Value result) {
+  LOG(INFO) << "[orion] ExecuteJavaScript: Execution completed";
+
+  if (result.is_none()) {
...
+}
+
+// Implementation of OrionClickCoordinatesFunction
+ExtensionFunction::ResponseAction OrionClickCoordinatesFunction::Run() {
...
+                                       &error_message);
+  if (!tab_info) {
+    LOG(ERROR) << "[orion] ClickCoordinates: " << error_message;
+    browser_os::InteractionResponse response;
...
+  
+  LOG(INFO) << "[orion] ClickCoordinates: Clicking at (" 
+            << params->x << ", " << params->y << ")";
+  
...
+  
+  LOG(INFO) << "[orion] ClickCoordinates: Result = " 
...
+}
+
+// Implementation of OrionTypeAtCoordinatesFunction  
+ExtensionFunction::ResponseAction OrionTypeAtCoordinatesFunction::Run() {
...
+                                       &error_message);
+  if (!tab_info) {
+    LOG(ERROR) << "[orion] TypeAtCoordinates: " << error_message;
+    browser_os::InteractionResponse response;
+    response.success = false;
+    return RespondNow(ArgumentList(
...
+  gfx::PointF click_point(params->x, params->y);
+  
+  LOG(INFO) << "[orion] TypeAtCoordinates: Clicking at (" 
...
+  response.success = success;
+  
+  LOG(INFO) << "[orion] TypeAtCoordinates: Result = " 
...
+}
+
+// OrionChoosePathFunction implementation
+
+namespace {
+
...
+}  // namespace
+
+OrionChoosePathFunction::OrionChoosePathFunction() = default;
+
+OrionChoosePathFunction::~OrionChoosePathFunction() {
...
+}
+
+ExtensionFunction::ResponseAction OrionChoosePathFunction::Run() {
...
+}
+
+void OrionChoosePathFunction::FileSelected(const ui::SelectedFileInfo& file,
+                                               int index) {
...
+}
+
+void OrionChoosePathFunction::FileSelectionCanceled() {
