...
+namespace api {
+
+OrionChangeDetector::OrionChangeDetector(content::WebContents* web_contents)
+    : content::WebContentsObserver(web_contents) {}
+
+OrionChangeDetector::~OrionChangeDetector() {
+  timeout_timer_.Stop();
+}
+
+// Static method for synchronous detection
+bool OrionChangeDetector::ExecuteWithDetection(
+    content::WebContents* web_contents,
+    std::function<void()> action,
+    base::TimeDelta timeout) {
+  auto detector = std::make_unique<OrionChangeDetector>(web_contents);
+  return detector->ExecuteAndWait(std::move(action), timeout);
+}
+
+// Static method for asynchronous detection
+void OrionChangeDetector::ExecuteWithDetectionAsync(
+    content::WebContents* web_contents,
+    std::function<void()> action,
+    base::OnceCallback<void(bool)> callback,
+    base::TimeDelta timeout) {
+  // Create detector on heap - it will delete itself when done
+  auto* detector = new OrionChangeDetector(web_contents);
+  detector->ExecuteAndNotify(std::move(action), std::move(callback), timeout);
+}
+
+void OrionChangeDetector::StartMonitoring() {
+  monitoring_ = true;
+  change_detected_ = false;
+  VLOG(1) << "[orion] Started monitoring for changes";
+}
+
+bool OrionChangeDetector::ExecuteAndWait(std::function<void()> action,
+                                             base::TimeDelta timeout) {
+  StartMonitoring();
+  
+  // Execute the action
...
+  if (change_detected_) {
+    VLOG(1) << "[orion] Change detected immediately";
+    return true;
+  }
+  
...
+                      base::BindOnce(&OrionChangeDetector::OnTimeout,
...
+  monitoring_ = false;
+  
+  VLOG(1) << "[orion] Change detection result: " << change_detected_;
+  return change_detected_;
+}
+
+void OrionChangeDetector::ExecuteAndNotify(
+    std::function<void()> action,
+    base::OnceCallback<void(bool)> callback,
+    base::TimeDelta timeout) {
+  StartMonitoring();
...
+  if (change_detected_) {
+    VLOG(1) << "[orion] Change detected immediately (async)";
...
+}
+
+void OrionChangeDetector::OnChangeDetected() {
...
+}
+
+void OrionChangeDetector::OnTimeout() {
+  VLOG(1) << "[orion] Change detection timeout";
+  monitoring_ = false;
+  
+  // If synchronous wait, quit the run loop
+  if (wait_callback_) {
...
+
+void OrionChangeDetector::AccessibilityEventReceived(
+    const ui::AXUpdatesAndEvents& details) {
+  if (!monitoring_) return;
+  
+  // Any accessibility event indicates a change
+  if (!details.updates.empty() || !details.events.empty()) {
+    VLOG(2) << "[orion] Accessibility event detected";
+    OnChangeDetected();
+  }
+}
+
+void OrionChangeDetector::DidFinishNavigation(
+    content::NavigationHandle* navigation_handle) {
+  if (!monitoring_) return;
+  
+  VLOG(2) << "[orion] Navigation detected";
+  OnChangeDetected();
+}
+
+void OrionChangeDetector::DOMContentLoaded(
+    content::RenderFrameHost* render_frame_host) {
+  if (!monitoring_) return;
+  
+  VLOG(2) << "[orion] DOM content loaded";
+  OnChangeDetected();
+}
+
+void OrionChangeDetector::OnFocusChangedInPage(
+    const content::FocusedNodeDetails& details) {
+  if (!monitoring_) return;
+
+  VLOG(2) << "[orion] Focus changed";
+  OnChangeDetected();
+}
+
+void OrionChangeDetector::DidOpenRequestedURL(
