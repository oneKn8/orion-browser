...
+  LOG(INFO) << "orion: BrowserosToggleSidePanelForTab called for tab_id="
...
+      !window || !web_contents) {
+    LOG(WARNING) << "orion: Tab not found for tab_id=" << tab_id;
...
+  if (!browser_window) {
+    LOG(WARNING) << "orion: No browser window for tab_id=" << tab_id;
...
+  }
+
+  LOG(INFO) << "orion: has_contextual_options=" << has_contextual_options
+            << " for tab_id=" << tab_id;
+
...
+      LOG(WARNING) << "orion: No side panel path configured for extension="
+                   << extension.id();
+      return base::unexpected(
...
+    }
+
+    LOG(INFO) << "orion: Auto-registering contextual panel for tab_id="
...
+    LOG(WARNING) << "orion: Side panel is disabled for tab_id=" << tab_id;
+    return base::unexpected(
+        base::StringPrintf("Side panel is disabled for tabId: %d", tab_id));
+  }
+
+  // Toggle the contextual panel.
+  LOG(INFO) << "orion: Calling ToggleContextualExtensionSidePanel for tab_id="
+            << tab_id;
...
+
+  LOG(INFO) << "orion: Toggle result: is_now_open=" << is_now_open
+            << " for tab_id=" << tab_id;
+
+  return is_now_open;
+}
...
+    int tab_id,
+    bool include_incognito_information) {
+  LOG(INFO) << "orion: BrowserosIsSidePanelOpenForTab called for tab_id="
+            << tab_id << ", extension=" << extension.id();
+
+  // Find the tab.
+  WindowController* window = nullptr;
...
+  api::side_panel::PanelOptions current_options = GetOptions(extension, tab_id);
+  if (!current_options.enabled.value_or(true)) {
+    LOG(INFO) << "orion: Panel is disabled for tab_id=" << tab_id
+              << ", returning false";
+    return false;
+  }
+
...
+
+  LOG(INFO) << "orion: IsOpen result: is_open=" << is_open
+            << " for tab_id=" << tab_id;
+
+  return is_open;
+}
