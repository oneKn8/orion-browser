...
@@ -0,0 +1,668 @@
+// Copyright 2024 Orion Authors. All rights reserved.
...
+  const char* kBaseURL = "https://cdn.orion-browser.local/";
+
...
+
+// Forward declare internal SparkleGlue methods used by OrionUserDriver.
...
+@end
+
+@implementation OrionUserDriver
+
...
+@property(nonatomic, strong) OrionUserDriver* userDriver;
...
+  _userDriver = [[OrionUserDriver alloc] init];
...
+
+#if !defined(OFFICIAL_BUILD)
+  if (cmd->HasSwitch(orion::kSparkleDryRun)) {
+    LOG(WARNING) << "Sparkle: DRY-RUN MODE enabled";
+    _dryRunMode = YES;
+  }
+
+  if (cmd->HasSwitch(orion::kSparkleSpoofVersion)) {
+    std::string version = cmd->GetSwitchValueASCII(orion::kSparkleSpoofVersion);
+    LOG(WARNING) << "Sparkle: Spoofing version as " << version;
+    _spoofedVersion = base::SysUTF8ToNSString(version);
+  }
+
+  if (cmd->HasSwitch(orion::kSparkleVerbose)) {
...
+  auto* cmd = base::CommandLine::ForCurrentProcess();
+  if (cmd && cmd->HasSwitch(orion::kSparkleForceCheck)) {
...
+  auto* cmd = base::CommandLine::ForCurrentProcess();
+  if (cmd && cmd->HasSwitch(orion::kSparkleUrl)) {
+    std::string url = cmd->GetSwitchValueASCII(orion::kSparkleUrl);
...
+  auto* cmd = base::CommandLine::ForCurrentProcess();
+  if (cmd && cmd->HasSwitch(orion::kSparkleSkipSignature)) {
