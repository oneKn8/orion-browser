...
+      browser_creator_->AddFirstRunTabs({GURL("chrome://orion-welcome")});
...
+  // Orion: Start AFTER CreateDevToolsProtocolHandler so that Orion's
+  // CDP handler replaces Chromium's (StartRemoteDebuggingServer is a global
+  // singleton — the last caller wins).
+  LOG(INFO) << "orion: Starting Orion server process";
+  orion::OrionServerManager::GetInstance()->Start();
+
...
+
+  // Orion: Stop the Orion server during shutdown
+  LOG(INFO) << "orion: Stopping Orion server process";
+  orion::OrionServerManager::GetInstance()->Shutdown();
+
