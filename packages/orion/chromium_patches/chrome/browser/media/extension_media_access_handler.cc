...
+// 9. Orion extensions (AI Side Panel and Bug Reporter)
...
+         extension->id() == "egfdjlfmgnehecnclamagfafdccgfndp" ||
+         // Orion extensions
+         orion::IsOrionExtension(extension->id());
 }
...
+  // For Orion extensions in sidepanel, allow audio for teach mode
+  if (orion::IsOrionExtension(extension->id())) {
