...
+
+// Preference names
+const char kThirdPartyLlmProvidersPref[] = "orion.third_party_llm.providers";
+const char kThirdPartyLlmSelectedProviderPref[] = "orion.third_party_llm.selected_provider";
+
...
+    LOG(INFO) << "[orion] Provider list size changed from " << previous_size
...
+  
+  orion_metrics::OrionMetrics::Log("llmchat.created");
+  
+  return container;
+}
+
...
+  if (!prefs) {
+    LOG(ERROR) << "[orion] Failed to get PrefService";
...
+      if (!item.is_dict()) {
+        LOG(WARNING) << "[orion] Invalid provider entry (not a dict), skipping";
+        continue;
+      }
+
...
+
+      if (!name || name->empty()) {
+        LOG(WARNING) << "[orion] Provider missing name, skipping";
+        continue;
+      }
+
+      if (!url || url->empty()) {
+        LOG(WARNING) << "[orion] Provider missing URL, skipping";
+        continue;
+      }
+
+      GURL provider_url(*url);
+      if (!provider_url.is_valid()) {
+        LOG(WARNING) << "[orion] Invalid provider URL: " << *url;
+        continue;
+      }
+
...
+  if (providers_.empty()) {
+    LOG(INFO) << "[orion] No providers in prefs, using defaults";
+    providers_ = GetDefaultProviders();
...
+    LOG(WARNING) << "[orion] Invalid selected provider index: " << selected_index
+                 << ", resetting to 0";
+    current_provider_index_ = 0;
+    prefs->SetInteger(kThirdPartyLlmSelectedProviderPref, 0);
+  } else {
...
+  PrefService* prefs = GetProfile()->GetPrefs();
+  if (!prefs) {
+    LOG(ERROR) << "[orion] Failed to get PrefService for saving";
+    return;
+  }
+
...
+    LOG(ERROR) << "[orion] Invalid provider index: " << new_provider_index;
+    return;
+  }
+
+  provider_change_in_progress_ = true;
+
+  orion_metrics::OrionMetrics::Log("llmchat.provider.changed");
+
+  if (owned_web_contents_) {
+    GURL current_url = owned_web_contents_->GetURL();
+    if (current_url.is_valid()) {
...
+  
+  orion_metrics::OrionMetrics::Log("llmchat.screenshot.captured");
+
...
+    
+    orion_metrics::OrionMetrics::Log("llmchat.content.copied");
+    
...
+  LOG(INFO) << "[orion] Media access requested for origin: "
...
+
+    LOG(INFO) << "[orion] Auto-granting media permission for trusted LLM: "
+              << origin_url.host();
+
...
+      VLOG(2) << "[orion] Media permission check for trusted LLM origin: "
...
+
+  VLOG(2) << "[orion] Media permission check for origin: "
+          << security_origin.Serialize()
+          << " type: " << static_cast<int>(type)
