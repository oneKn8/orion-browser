...
+  LOG(INFO) << "orion: IsContextualExtensionSidePanelOpen called for extension="
+            << extension_id;
+
+  if (!browser_window || !web_contents) {
+    LOG(WARNING) << "orion: browser_window or web_contents is null";
+    return false;
+  }
+
...
+  LOG(INFO) << "orion: is_active_tab=" << is_active_tab;
+
...
+    LOG(INFO) << "orion: side_panel is_showing=" << is_showing;
+    if (!is_showing) {
+      return false;
+    }
...
+    LOG(INFO) << "orion: contextual panel is_active=" << is_active;
+    return is_active;
+  }
+
...
+      tab->GetTabFeatures()->side_panel_registry();
+  bool is_active = IsKeyActiveInRegistry(contextual_registry, extension_key);
+  LOG(INFO) << "orion: inactive tab contextual panel is_active=" << is_active;
+  return is_active;
+}
+
...
+  LOG(INFO) << "orion: ToggleContextualExtensionSidePanel called for extension="
...
+  }
+
+  LOG(INFO) << "orion: is_currently_open=" << is_currently_open
...
+  }
+
+  LOG(INFO) << "orion: should_open=" << should_open;
+
+  // If already in desired state, return early.
+  if (should_open == is_currently_open) {
+    LOG(INFO) << "orion: Already in desired state, no action needed";
+    return is_currently_open;
+  }
+
+  if (!should_open) {
+    LOG(INFO) << "orion: Closing contextual panel";
...
+    return false;
+  } else {
+    LOG(INFO) << "orion: Opening contextual panel";
+
+    SidePanelEntry* contextual_entry =
+        contextual_registry->GetEntryForKey(extension_key);
+    LOG(INFO) << "orion: Got contextual_entry: "
+              << (contextual_entry ? "yes" : "no");
+
+    if (!contextual_entry) {
+      LOG(WARNING) << "orion: No contextual entry found, cannot open";
+      return false;
+    }
+
...
+
+    if (is_active_tab) {
+      LOG(INFO) << "orion: Calling side_panel_ui->Show() for active tab";
