...
+        LOG(INFO) << "orion: Toolbar action clicked for extension="
+                  << extension_id;
+
+        // Get the active tab.
...
+        if (!active_contents) {
+          LOG(WARNING) << "orion: No active tab contents";
+          return;
+        }
+
+        int tab_id = extensions::ExtensionTabUtil::GetTabId(active_contents);
+        LOG(INFO) << "orion: Active tab_id=" << tab_id;
+
...
+
+        if (!extension) {
+          LOG(WARNING) << "orion: Extension not found: " << extension_id;
+          return;
+        }
+
...
+
+        if (!result.has_value()) {
+          LOG(WARNING) << "orion: Toggle failed: " << result.error();
+        } else {
+          LOG(INFO) << "orion: Toggle result: " << result.value();
+        }
+      },
+      extension_id, bwi);
+}
