...
+
+// Preference names
+const char kClashOfGptsPaneProvidersPref[] = "orion.clash_of_gpts.pane_providers";  // Per-pane selections
+const char kClashOfGptsLastUrlsPref[] = "orion.clash_of_gpts.last_urls";
+const char kClashOfGptsPaneCountPref[] = "orion.clash_of_gpts.pane_count";
+
...
+    widget_->Show();
+    widget_->Activate();
+    orion_metrics::OrionMetrics::Log("llmhub.shown");
+  }
+}
+
...
+    LOG(ERROR) << "[orion] Invalid provider index: " << provider_index;
+    return;
+  }
+
...
+  SaveState();
+  
+  orion_metrics::OrionMetrics::Log("llmhub.panecount.changed", 
