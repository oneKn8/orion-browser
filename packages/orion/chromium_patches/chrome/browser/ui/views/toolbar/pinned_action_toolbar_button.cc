...
+  // Set text from action item if available for Orion actions
+  if (auto* action_item = container_->GetActionItemFor(action_id)) {
+    if (orion::IsOrionAction(action_id)) {
+      // Check if labels should be shown
+      bool show_labels = true;
+      if (browser_ && browser_->profile()) {
+        show_labels = orion::ShouldShowToolbarLabels(
...
+  if (!orion::IsOrionAction(action_id_)) {
+    return;
+  }
+
...
+  if (browser_ && browser_->profile()) {
+    show_labels =
+        orion::ShouldShowToolbarLabels(browser_->profile()->GetPrefs());
+  }
+
+  if (show_labels) {
+    views::LabelButton::SetText(action_item->GetText());
...
+  // Update the text from the action item for Orion actions
+  if (orion::IsOrionAction(action_view_->GetActionId())) {
+    // Check if labels should be shown
+    bool show_labels = true;
