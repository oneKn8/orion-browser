...
+  }
+
+  // Orion Agent - toggles contextual side panel on active tab.
...
+                  LOG(WARNING) << "orion: No active tab for Agent action";
+                  return;
+                }
+
...
+                        ->enabled_extensions()
+                        .GetByID(orion::kAgentV2ExtensionId);
+                if (!extension) {
+                  LOG(WARNING) << "orion: Agent extension not found";
...
+                            ORION_AGENT_INSTALLING_INFOBAR_DELEGATE,
+                        nullptr,
+                        u"Orion Agent is installing/updating. Please try again shortly.",
+                        /*auto_expire=*/true,
+                        /*should_animate=*/true,
+                        /*closeable=*/true);
+                  }
...
+                LOG(INFO) << "orion: Agent toolbar action for tab_id="
+                          << tab_id;
+
+                extensions::SidePanelService* service =
+                    extensions::SidePanelService::Get(profile);
+                if (!service) {
+                  LOG(WARNING) << "orion: SidePanelService not found";
+                  return;
+                }
+
...
+
+                if (!result.has_value()) {
+                  LOG(WARNING) << "orion: Agent toggle failed: "
+                               << result.error();
+                } else {
+                  LOG(INFO) << "orion: Agent toggle result: "
+                            << result.value();
+                }
+              },
+              bwi))
+          .SetActionId(kActionOrionAgent)
+          .SetText(u"Assistant")
+          .SetTooltipText(u"Ask Orion")
