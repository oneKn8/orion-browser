...
+  // Check if it's a Orion extension
+  if (orion::IsOrionPinnedExtension(action_id)) {
+    return true;
+  }
+
   auto* management =
...
+  // Add Orion extensions to the force-pinned list (only those marked as pinned)
+  for (const std::string& ext_id : orion::GetOrionExtensionIds()) {
+    if (orion::IsOrionPinnedExtension(ext_id) &&
