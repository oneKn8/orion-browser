...
+#include "chrome/browser/orion/core/orion_action_utils.h"
+#include "chrome/browser/orion/core/orion_prefs.h"
...
+  // Observe Orion visibility prefs for reactive updates.
+  pref_change_registrar_.Add(
+      orion::prefs::kShowLLMChat,
+      base::BindRepeating(
+          &PinnedToolbarActionsModel::OnOrionVisibilityPrefChanged,
+          base::Unretained(this)));
+  pref_change_registrar_.Add(
+      orion::prefs::kShowLLMHub,
+      base::BindRepeating(
+          &PinnedToolbarActionsModel::OnOrionVisibilityPrefChanged,
+          base::Unretained(this)));
+  pref_change_registrar_.Add(
+      orion::prefs::kShowToolbarLabels,
+      base::BindRepeating(
+          &PinnedToolbarActionsModel::OnOrionLabelsPrefChanged,
+          base::Unretained(this)));
+
...
+  }
+
+  // Pin native Orion actions if:
+  // 1. Their feature flag is enabled (or no feature flag exists)
+  // 2. Their visibility pref allows it
+  for (actions::ActionId id : orion::kOrionNativeActionIds) {
+    const base::Feature* feature = orion::GetFeatureForOrionAction(id);
+    bool feature_enabled = !feature || base::FeatureList::IsEnabled(*feature);
+    bool pref_enabled = orion::ShouldShowToolbarAction(id, pref_service_);
+
...
+
+void PinnedToolbarActionsModel::OnOrionVisibilityPrefChanged() {
+  // Re-evaluate which Orion actions should be pinned.
...
+}
+
+void PinnedToolbarActionsModel::OnOrionLabelsPrefChanged() {
