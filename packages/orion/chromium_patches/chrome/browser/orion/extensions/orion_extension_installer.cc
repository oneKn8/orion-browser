diff --git a/chrome/browser/orion/extensions/orion_extension_installer.cc b/chrome/browser/orion/extensions/orion_extension_installer.cc
new file mode 100644
index 0000000000000..ad2da65f1d4f4
--- /dev/null
+++ b/chrome/browser/orion/extensions/orion_extension_installer.cc
@@ -0,0 +1,313 @@
+// Copyright 2024 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#include "chrome/browser/orion/extensions/orion_extension_installer.h"
+
+#include <optional>
+#include <utility>
+
...
+
+OrionExtensionInstaller::OrionExtensionInstaller(Profile* profile)
+    : profile_(profile) {
+  for (const std::string& id : GetOrionExtensionIds()) {
+    extension_ids_.insert(id);
+  }
+}
+
+OrionExtensionInstaller::~OrionExtensionInstaller() = default;
+
+void OrionExtensionInstaller::StartInstallation(
+    const GURL& config_url,
+    InstallCompleteCallback callback) {
+  config_url_ = config_url;
+  callback_ = std::move(callback);
+
+  LOG(INFO) << "orion: Starting extension installation";
+
...
+}
+
+bool OrionExtensionInstaller::TryLoadFromBundled() {
+  base::FilePath bundled_path;
+  if (!base::PathService::Get(chrome::DIR_ORION_BUNDLED_EXTENSIONS,
+                              &bundled_path)) {
+    LOG(INFO) << "orion: Bundled path not available";
+    return false;
+  }
+
...
+    LOG(INFO) << "orion: No bundled manifest at " << manifest_path.value();
+    return false;
+  }
+
+  LOG(INFO) << "orion: Loading from bundled at " << bundled_path.value();
+
+  base::ThreadPool::PostTaskAndReplyWithResult(
+      FROM_HERE, {base::MayBlock(), base::TaskPriority::USER_BLOCKING},
+      base::BindOnce(&OrionExtensionInstaller::ReadBundledManifest,
+                     manifest_path, bundled_path),
+      base::BindOnce(&OrionExtensionInstaller::OnBundledLoadComplete,
...
+
+// static
+base::DictValue OrionExtensionInstaller::ReadBundledManifest(
+    const base::FilePath& manifest_path,
+    const base::FilePath& bundled_path) {
+  std::string json_content;
+  if (!base::ReadFileToString(manifest_path, &json_content)) {
+    LOG(ERROR) << "orion: Failed to read bundled manifest";
...
+    LOG(ERROR) << "orion: Invalid bundled manifest JSON";
+    return base::DictValue();
+  }
+
...
+    }
+
+    // Only install registered Orion extensions
+    if (!IsOrionExtension(extension_id)) {
+      LOG(WARNING) << "orion: Skipping unregistered extension "
+                   << extension_id;
+      continue;
+    }
+
...
+
+    if (!crx_file || !version) {
+      LOG(WARNING) << "orion: Bundled config missing crx/version for "
+                   << extension_id;
+      continue;
+    }
+
...
+      LOG(WARNING) << "orion: CRX not found: " << crx_path.value();
+      continue;
+    }
+
...
+    LOG(INFO) << "orion: Prepared bundled " << extension_id << " v"
+              << *version;
+  }
+
+  return prefs;
+}
+
+void OrionExtensionInstaller::OnBundledLoadComplete(
+    const base::FilePath& bundled_path,
+    base::DictValue prefs) {
+  LOG(INFO) << "orion: Bundled load complete, " << prefs.size()
+            << " extensions from " << bundled_path.value();
+
+  if (prefs.empty()) {
+    LOG(INFO) << "orion: No bundled prefs, falling back to remote";
+    FetchFromRemote();
+    return;
+  }
+
...
+}
+
+void OrionExtensionInstaller::FetchFromRemote() {
+  if (!config_url_.is_valid()) {
+    LOG(ERROR) << "orion: Invalid config URL";
+    Complete(InstallResult());
+    return;
+  }
+
+  LOG(INFO) << "orion: Fetching config from " << config_url_.spec();
+
...
+      base::BindOnce(&OrionExtensionInstaller::OnRemoteFetchComplete,
+                     weak_ptr_factory_.GetWeakPtr()));
+}
+
+void OrionExtensionInstaller::OnRemoteFetchComplete(
+    std::optional<std::string> response_body) {
+  if (!response_body.has_value()) {
+    LOG(ERROR) << "orion: Failed to fetch config";
+    Complete(InstallResult());
+    return;
+  }
+
...
+  }
+
+  LOG(INFO) << "orion: Loaded " << result.prefs.size()
+            << " extensions from remote config";
+
+  Complete(std::move(result));
+}
+
+base::DictValue OrionExtensionInstaller::ParseConfigJson(
+    const std::string& json_content) {
+  std::optional<base::Value> parsed =
+      base::JSONReader::Read(json_content, base::JSON_PARSE_RFC);
+
+  if (!parsed || !parsed->is_dict()) {
+    LOG(ERROR) << "orion: Invalid config JSON";
+    return base::DictValue();
+  }
+
...
+
+  if (!extensions) {
+    LOG(ERROR) << "orion: No 'extensions' key in config";
+    return base::DictValue();
+  }
+
...
+}
+
+void OrionExtensionInstaller::Complete(InstallResult result) {
+  if (callback_) {
