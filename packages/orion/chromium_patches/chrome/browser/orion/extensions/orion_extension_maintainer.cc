diff --git a/chrome/browser/orion/extensions/orion_extension_maintainer.cc b/chrome/browser/orion/extensions/orion_extension_maintainer.cc
new file mode 100644
index 0000000000000..bb33ae5d3b156
--- /dev/null
+++ b/chrome/browser/orion/extensions/orion_extension_maintainer.cc
@@ -0,0 +1,395 @@
+// Copyright 2024 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#include "chrome/browser/orion/extensions/orion_extension_maintainer.h"
+
+#include <optional>
+#include <utility>
+
...
+    net::DefineNetworkTrafficAnnotation("orion_extension_maintenance", R"(
+        semantics {
+          sender: "Orion Extension Maintainer"
+          description:
+            "Fetches JSON configuration for Orion extension maintenance."
...
+}  // namespace
+
+OrionExtensionMaintainer::OrionExtensionMaintainer(Profile* profile)
+    : profile_(profile) {}
+
+OrionExtensionMaintainer::~OrionExtensionMaintainer() = default;
+
+void OrionExtensionMaintainer::Start(const GURL& config_url,
...
+
+  LOG(INFO) << "orion: Maintainer started, " << extension_ids_.size()
+            << " extensions, scheduling in "
+            << kInitialMaintenanceDelay.InSeconds() << "s";
+
+  base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
+      FROM_HERE,
+      base::BindOnce(&OrionExtensionMaintainer::RunMaintenanceCycle,
+                     weak_ptr_factory_.GetWeakPtr()),
+      kInitialMaintenanceDelay);
+}
+
+void OrionExtensionMaintainer::UpdateExtensionIds(
+    std::set<std::string> ids) {
+  extension_ids_ = std::move(ids);
+}
+
+void OrionExtensionMaintainer::RunMaintenanceCycle() {
+  if (!profile_) {
...
+      base::BindOnce(&OrionExtensionMaintainer::OnConfigFetched,
+                     weak_ptr_factory_.GetWeakPtr(), std::move(loader)));
+}
+
+void OrionExtensionMaintainer::OnConfigFetched(
...
+      }
+
+      LOG(INFO) << "orion: Updated config with " << last_config_.size()
+                << " extensions";
+    } else {
+      LOG(WARNING) << "orion: Fetched config parsed as empty";
+    }
+  } else {
+    LOG(WARNING) << "orion: Failed to fetch maintenance config";
+  }
+
+  ExecuteMaintenanceTasks();
+  ScheduleNextMaintenance();
+}
+
+base::DictValue OrionExtensionMaintainer::ParseConfigJson(
...
+}
+
+void OrionExtensionMaintainer::ExecuteMaintenanceTasks() {
+  LOG(INFO) << "orion: Executing maintenance tasks";
+
...
+}
+
+void OrionExtensionMaintainer::ScheduleNextMaintenance() {
+  base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
+      FROM_HERE,
+      base::BindOnce(&OrionExtensionMaintainer::RunMaintenanceCycle,
+                     weak_ptr_factory_.GetWeakPtr()),
...
+}
+
+void OrionExtensionMaintainer::UninstallDeprecatedExtensions() {
...
+    }
+
+    LOG(INFO) << "orion: Uninstalling deprecated extension " << id;
+
+    std::u16string error;
+    if (!registrar->UninstallExtension(
+            id, extensions::UNINSTALL_REASON_ORPHANED_EXTERNAL_EXTENSION,
+            &error)) {
+      LOG(WARNING) << "orion: Failed to uninstall " << id << ": " << error;
+    }
+  }
+}
+
+void OrionExtensionMaintainer::ReinstallMissingExtensions() {
+  if (!profile_ || last_config_.empty()) {
+    return;
+  }
+
...
+    }
+
+    LOG(INFO) << "orion: Reinstalling missing extension " << id;
+
...
+}
+
+void OrionExtensionMaintainer::ReenableDisabledExtensions() {
+  if (!profile_) {
+    return;
+  }
+
...
+    }
+
+    LOG(INFO) << "orion: Re-enabling disabled extension " << id;
+    registrar->EnableExtension(id);
+  }
+}
+
+void OrionExtensionMaintainer::ForceUpdateCheck() {
...
+  }
+
+  LOG(INFO) << "orion: Force update check for "
...
+    if (ext) {
+      LOG(INFO) << "orion: ext=" << id
+                << " v" << ext->version().GetString();
+    } else {
+      LOG(INFO) << "orion: ext=" << id << " not installed";
+    }
+  }
+
+  extensions::ExtensionUpdater::CheckParams params;
...
+}
+
+void OrionExtensionMaintainer::LogExtensionHealth(
...
+
+    orion_metrics::OrionMetrics::Log("ota.extension.unexpected_state",
+                                             std::move(properties));
+
+    LOG(WARNING) << "orion: Extension " << id << " in state: " << state
+                 << " (context: " << context << ")";
+  }
+}
+
