diff --git a/chrome/browser/orion/extensions/orion_extension_loader.cc b/chrome/browser/orion/extensions/orion_extension_loader.cc
new file mode 100644
index 0000000000000..e61b45d08b7e2
--- /dev/null
+++ b/chrome/browser/orion/extensions/orion_extension_loader.cc
@@ -0,0 +1,226 @@
...
+}  // namespace
+
+OrionExtensionLoader::OrionExtensionLoader(Profile* profile)
+    : profile_(profile) {
+  config_url_ =
+      GURL(base::FeatureList::IsEnabled(features::kBrowserOsAlphaFeatures)
+               ? kOrionAlphaConfigUrl
+               : kOrionConfigUrl);
+
...
+}
+
+OrionExtensionLoader::~OrionExtensionLoader() = default;
+
+void OrionExtensionLoader::SetConfigUrl(const GURL& url) {
+  config_url_ = url;
+}
+
+void OrionExtensionLoader::StartLoading() {
+  LOG(INFO) << "orion: Extension loader starting";
+
+  installer_ = std::make_unique<OrionExtensionInstaller>(profile_);
+  maintainer_ = std::make_unique<OrionExtensionMaintainer>(profile_);
+
+  installer_->StartInstallation(
+      config_url_,
+      base::BindOnce(&OrionExtensionLoader::OnInstallComplete,
+                     weak_ptr_factory_.GetWeakPtr()));
+}
+
+void OrionExtensionLoader::OnInstallComplete(InstallResult result) {
+  LOG(INFO) << "orion: OnInstallComplete from_bundled="
...
+}
+
+const base::FilePath OrionExtensionLoader::GetBaseCrxFilePath() {
+  return bundled_crx_base_path_;
+}
+
+void OrionExtensionLoader::OnStartupComplete(bool from_bundled) {
+  LOG(INFO) << "orion: Startup complete (from_bundled=" << from_bundled
+            << ")";
+
+  if (from_bundled) {
...
+        FROM_HERE,
+        base::BindOnce(
+            &OrionExtensionLoader::InstallBundledExtensionsNow,
+            weak_ptr_factory_.GetWeakPtr()),
+        kImmediateInstallDelay);
+  } else {
+    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
+        FROM_HERE,
+        base::BindOnce(&OrionExtensionLoader::InstallRemoteExtensionsNow,
...
+}
+
+void OrionExtensionLoader::InstallRemoteExtensionsNow(
...
+  }
+
+  LOG(INFO) << "orion: Installing " << extension_ids_.size()
...
+}
+
+void OrionExtensionLoader::InstallBundledExtensionsNow() {
...
+
+    LOG(INFO) << "orion: Installing bundled " << id << " v" << it->second;
+
