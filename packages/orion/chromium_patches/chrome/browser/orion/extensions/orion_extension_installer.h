diff --git a/chrome/browser/orion/extensions/orion_extension_installer.h b/chrome/browser/orion/extensions/orion_extension_installer.h
new file mode 100644
index 0000000000000..9a3c2000ed05a
--- /dev/null
+++ b/chrome/browser/orion/extensions/orion_extension_installer.h
@@ -0,0 +1,100 @@
+// Copyright 2024 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#ifndef CHROME_BROWSER_ORION_EXTENSIONS_ORION_EXTENSION_INSTALLER_H_
+#define CHROME_BROWSER_ORION_EXTENSIONS_ORION_EXTENSION_INSTALLER_H_
+
+#include <memory>
+#include <optional>
+#include <set>
...
+};
+
+// Handles one-time initial installation of Orion extensions.
+// Tries bundled CRX files first, falls back to remote config.
+class OrionExtensionInstaller {
+ public:
+  using InstallCompleteCallback =
+      base::OnceCallback<void(InstallResult result)>;
+
+  explicit OrionExtensionInstaller(Profile* profile);
+  ~OrionExtensionInstaller();
+
+  OrionExtensionInstaller(const OrionExtensionInstaller&) = delete;
+  OrionExtensionInstaller& operator=(const OrionExtensionInstaller&) =
+      delete;
+
...
+
+  base::WeakPtrFactory<OrionExtensionInstaller> weak_ptr_factory_{this};
+};
+
+}  // namespace orion
+
+#endif  // CHROME_BROWSER_ORION_EXTENSIONS_ORION_EXTENSION_INSTALLER_H_
