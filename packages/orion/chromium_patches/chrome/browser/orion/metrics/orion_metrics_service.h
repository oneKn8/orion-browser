diff --git a/chrome/browser/orion/metrics/orion_metrics_service.h b/chrome/browser/orion/metrics/orion_metrics_service.h
new file mode 100644
index 0000000000000..beadbcfd183e0
--- /dev/null
+++ b/chrome/browser/orion/metrics/orion_metrics_service.h
@@ -0,0 +1,96 @@
+// Copyright 2025 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#ifndef CHROME_BROWSER_ORION_METRICS_ORION_METRICS_SERVICE_H_
+#define CHROME_BROWSER_ORION_METRICS_ORION_METRICS_SERVICE_H_
+
+#include <memory>
+#include <optional>
+#include <string>
...
+class OrionMetricsService : public KeyedService {
+ public:
+  explicit OrionMetricsService(
+      PrefService* pref_service,
+      PrefService* local_state_prefs,
+      scoped_refptr<network::SharedURLLoaderFactory> url_loader_factory);
+
+  OrionMetricsService(const OrionMetricsService&) = delete;
+  OrionMetricsService& operator=(const OrionMetricsService&) = delete;
+
+  ~OrionMetricsService() override;
+
...
+  base::WeakPtrFactory<OrionMetricsService> weak_factory_{this};
+};
+
+}  // namespace orion_metrics
+
+#endif  // CHROME_BROWSER_ORION_METRICS_ORION_METRICS_SERVICE_H_
