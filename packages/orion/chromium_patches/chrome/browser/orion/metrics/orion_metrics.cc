diff --git a/chrome/browser/orion/metrics/orion_metrics.cc b/chrome/browser/orion/metrics/orion_metrics.cc
new file mode 100644
index 0000000000000..cf1a9a0c1790b
--- /dev/null
+++ b/chrome/browser/orion/metrics/orion_metrics.cc
@@ -0,0 +1,100 @@
...
+OrionMetricsService* GetMetricsService() {
...
+  return OrionMetricsServiceFactory::GetForBrowserContext(profile);
+}
+
...
+  } else {
+    VLOG(1) << "orion: Metrics service not available for event: " << event_name;
+  }
+}
+
+}  // namespace
+
+// static
+void OrionMetrics::Log(const std::string& event_name, double sample_rate) {
+  Log(event_name, base::DictValue(), sample_rate);
+}
+
+// static
+void OrionMetrics::Log(const std::string& event_name,
...
+
+// static
+void OrionMetrics::Log(const std::string& event_name, base::DictValue properties,
+                           double sample_rate) {
