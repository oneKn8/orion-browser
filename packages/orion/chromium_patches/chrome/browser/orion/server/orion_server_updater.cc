diff --git a/chrome/browser/orion/server/orion_server_updater.cc b/chrome/browser/orion/server/orion_server_updater.cc
new file mode 100644
index 0000000000000..9050130727fc8
--- /dev/null
+++ b/chrome/browser/orion/server/orion_server_updater.cc
@@ -0,0 +1,1078 @@
...
+#include "chrome/browser/orion/server/orion_server_constants.h"
...
+  return net::DefineNetworkTrafficAnnotation("orion_server_appcast", R"(
+    semantics {
+      sender: "Orion Server Updater"
+      description:
+        "Checks for updates to the Orion server component by fetching "
+        "an appcast XML feed."
...
+      internal {
+        contacts {
+          email: "nikhil@orion-browser.local"
+        }
+      }
+    }
+    policy {
+      cookies_allowed: NO
+      setting: "This feature can be disabled via --disable-orion-server or --disable-orion-server-updater."
+      policy_exception_justification:
+        "Essential for keeping Orion server component up to date."
+    })");
+}
+
+net::NetworkTrafficAnnotationTag GetDownloadTrafficAnnotation() {
+  return net::DefineNetworkTrafficAnnotation("orion_server_download", R"(
+    semantics {
+      sender: "Orion Server Updater"
+      description:
+        "Downloads a new version of the Orion server component."
...
+  return net::DefineNetworkTrafficAnnotation("orion_server_status", R"(
+    semantics {
+      sender: "Orion Server Updater"
+      description:
+        "Checks if the local Orion server is ready for hot-swap update."
...
+      setting: "This feature can be disabled via --disable-orion-server or --disable-orion-server-updater."
+      policy_exception_justification:
+        "Essential for coordinating Orion server updates."
+    })");
+}
+
...
+    LOG(ERROR) << "orion: Failed to decode public key from base64";
+    return false;
+  }
+  if (public_key_bytes.size() != ED25519_PUBLIC_KEY_LEN) {
+    LOG(ERROR) << "orion: Invalid public key length: "
...
+    LOG(ERROR) << "orion: Failed to decode signature from base64";
+    return false;
+  }
+  if (signature_bytes.size() != ED25519_SIGNATURE_LEN) {
+    LOG(ERROR) << "orion: Invalid signature length: "
...
+    LOG(ERROR) << "orion: Failed to read file for signature verification: "
+               << file_path;
+    return false;
+  }
+
...
+  if (result != 1) {
+    LOG(ERROR) << "orion: Ed25519 signature verification failed";
+    return false;
+  }
+
+  LOG(INFO) << "orion: Ed25519 signature verified successfully";
+  return true;
+}
+
...
+  }
+
+  LOG(INFO) << "orion: Extracted ZIP to " << dest_dir;
+  return "";  // Success
+}
+
...
+    LOG(WARNING) << "orion: Cleaning stale version directory: " << dest_dir;
...
+}  // namespace
+
+OrionServerUpdater::OrionServerUpdater(
+    orion::OrionServerManager* manager)
+    : manager_(manager) {}
+
+OrionServerUpdater::~OrionServerUpdater() {
+  Stop();
+}
+
+void OrionServerUpdater::Start() {
+  LOG(INFO) << "orion: Starting server updater";
+
...
+                            &OrionServerUpdater::OnUpdateTimer);
+}
+
+void OrionServerUpdater::LoadVersionCachesAsync() {
...
+          },
+          version_file),
+      base::BindOnce(&OrionServerUpdater::OnDownloadedVersionLoaded,
...
+          bundled_binary),
+      base::BindOnce(
+          [](base::WeakPtr<OrionServerUpdater> self,
...
+}
+
+void OrionServerUpdater::OnDownloadedVersionLoaded(
+    const std::string& version_str) {
+  if (!version_str.empty()) {
+    cached_downloaded_version_ = base::Version(version_str);
+    LOG(INFO) << "orion: Cached downloaded version: "
...
+}
+
+void OrionServerUpdater::OnBundledVersionLoaded(int exit_code,
...
+      LOG(INFO) << "orion: Cached bundled version: "
+                << cached_bundled_version_.GetString();
+    } else {
+      LOG(WARNING) << "orion: Could not parse bundled version from: "
+                   << output;
+    }
+  } else {
+    LOG(WARNING) << "orion: Failed to get bundled version (exit_code="
+                 << exit_code << ")";
+  }
...
+}
+
+void OrionServerUpdater::CheckVersionCachesAndStart() {
...
+}
+
+void OrionServerUpdater::Stop() {
+  LOG(INFO) << "orion: Stopping server updater";
...
+}
+
+bool OrionServerUpdater::IsUpdateInProgress() const {
+  return update_in_progress_;
+}
+
+void OrionServerUpdater::CheckNow() {
+  if (!bundled_version_loaded_ || !downloaded_version_loaded_) {
+    LOG(INFO) << "orion: Version caches not loaded yet, skipping check";
+    return;
+  }
+
+  if (update_in_progress_) {
+    LOG(INFO) << "orion: Update check already in progress, skipping";
+    return;
+  }
+
+  FetchAppcast();
+}
+
+void OrionServerUpdater::OnUpdateTimer() {
+  CheckNow();
+}
+
+void OrionServerUpdater::FetchAppcast() {
...
+  if (cmd->HasSwitch(orion::kServerAppcastUrl)) {
+    appcast_url = cmd->GetSwitchValueASCII(orion::kServerAppcastUrl);
+    LOG(INFO) << "orion: Using custom appcast URL: " << appcast_url;
...
+  }
+
+  LOG(INFO) << "orion: Fetching appcast from " << url;
+
...
+      url_loader_factory,
+      base::BindOnce(&OrionServerUpdater::OnAppcastFetched,
+                     weak_factory_.GetWeakPtr()),
+      kMaxAppcastSize);
+}
+
+void OrionServerUpdater::OnAppcastFetched(
...
+  }
+
+  LOG(INFO) << "orion: Received appcast (" << response->size() << " bytes)";
+
+  // Parse the appcast
+  std::optional<AppcastItem> item =
+      OrionAppcastParser::ParseLatestItem(*response);
+  if (!item) {
+    OnError("check", "Failed to parse appcast XML");
+    return;
+  }
+
+  LOG(INFO) << "orion: Latest version in appcast: "
...
+  }
+
+  LOG(INFO) << "orion: Found enclosure for current platform: "
+            << enclosure->url;
+
+  // Compare with current version
+  base::Version current = GetCurrentVersion();
+  LOG(INFO) << "orion: Current version: "
+            << (current.IsValid() ? current.GetString() : "(none)");
+
+  if (current.IsValid() && current >= item->version) {
+    LOG(INFO) << "orion: Already up to date";
+    ResetState();
+    return;
+  }
+
+  LOG(INFO) << "orion: New version available: "
+            << item->version.GetString();
+  pending_item_ = *item;
...
+}
+
+void OrionServerUpdater::CheckVersionAlreadyDownloaded(
...
+      base::BindOnce(&OrionServerUpdater::OnVersionExistsCheck,
+                     weak_factory_.GetWeakPtr(), enclosure, version));
+}
+
+void OrionServerUpdater::OnVersionExistsCheck(
+    const AppcastEnclosure& enclosure,
+    const base::Version& version,
+    bool exists) {
+  if (exists) {
+    LOG(INFO) << "orion: Version " << version.GetString()
...
+}
+
+void OrionServerUpdater::StartDownload(const AppcastEnclosure& enclosure,
+                                           const base::Version& version) {
...
+            LOG(INFO) << "orion: Downloading " << download_url;
+
+            auto request = std::make_unique<network::ResourceRequest>();
...
+                url_loader_factory,
+                base::BindOnce(&OrionServerUpdater::OnDownloadComplete,
+                               self, ver),
+                download_path);
+          },
+          weak_factory_.GetWeakPtr(), enclosure, version));
+}
+
+void OrionServerUpdater::OnDownloadComplete(const base::Version& version,
...
+  }
+
+  LOG(INFO) << "orion: Download complete: " << zip_path;
+
+  // Now verify and extract
+  VerifyAndExtract(zip_path, pending_signature_, version);
+}
+
+void OrionServerUpdater::VerifyAndExtract(const base::FilePath& zip_path,
+                                              const std::string& signature,
+                                              const base::Version& version) {
...
+
+  LOG(INFO) << "orion: Verifying signature and extracting to " << dest_dir;
+
...
+      base::BindOnce(
+          [](base::WeakPtr<OrionServerUpdater> self, base::Version version,
+             VerifyExtractResult result) {
+            if (!self) {
+              return;
+            }
...
+}
+
+void OrionServerUpdater::OnVerifyAndExtractComplete(
+    const base::Version& version,
+    bool success,
...
+  }
+
+  LOG(INFO) << "orion: Verification and extraction successful";
+
+  // Test the binary
+  TestBinary(version);
+}
+
+void OrionServerUpdater::TestBinary(const base::Version& version) {
+  state_ = State::kTesting;
+
+  base::FilePath binary_path = GetDownloadedBinaryPath(version);
+  LOG(INFO) << "orion: Testing binary: " << binary_path;
+
...
+}
+
+void OrionServerUpdater::OnBinaryTestComplete(const base::Version& version,
+                                                  int exit_code,
+                                                  const std::string& output) {
+  if (exit_code != 0) {
+    LOG(ERROR) << "orion: Binary test failed with exit code " << exit_code
+               << ": " << output;
+
...
+  }
+
+  LOG(INFO) << "orion: Binary test passed: " << output;
+
+  // Check if server is ready for hot-swap
+  CheckServerStatus();
+}
+
+void OrionServerUpdater::CheckServerStatus() {
+  GURL status_url("http://127.0.0.1:" +
+                  base::NumberToString(manager_->GetServerPort()) + "/status");
+
+  LOG(INFO) << "orion: Checking server status at " << status_url;
+
+  auto request = std::make_unique<network::ResourceRequest>();
...
+      url_loader_factory,
+      base::BindOnce(&OrionServerUpdater::OnStatusFetched,
+                     weak_factory_.GetWeakPtr()),
+      4096);
+}
+
+void OrionServerUpdater::OnStatusFetched(
+    std::optional<std::string> response) {
+  if (!response.has_value()) {
+    int net_error = status_loader_->NetError();
+    LOG(WARNING) << "orion: Failed to fetch server status: "
...
+    LOG(WARNING)
+        << "orion: Invalid status response, proceeding with update";
+    OnServerStatusChecked(/*can_update=*/true);
+    return;
+  }
+
...
+    LOG(WARNING) << "orion: Status response missing can_update field";
+    OnServerStatusChecked(/*can_update=*/true);
+    return;
+  }
+
...
+}
+
+void OrionServerUpdater::OnServerStatusChecked(bool can_update) {
+  if (!can_update) {
+    LOG(INFO) << "orion: Server busy, will retry hot-swap at next check";
+
+    base::DictValue props;
+    props.Set("pending_version", pending_item_.version.GetString());
+    orion_metrics::OrionMetrics::Log("server.ota.busy",
+                                             std::move(props));
+
+    ResetState();
+    return;
...
+}
+
+void OrionServerUpdater::PerformHotSwap(const base::Version& version) {
+  LOG(INFO) << "orion: Performing hot-swap to version "
+            << version.GetString();
+
...
+      base::BindOnce(&OrionServerUpdater::OnHotSwapComplete,
+                     weak_factory_.GetWeakPtr(), old_version, version));
+}
+
+void OrionServerUpdater::OnHotSwapComplete(const base::Version& old_version,
+                                               const base::Version& new_version,
+                                               bool success) {
+  if (!success) {
+    LOG(ERROR) << "orion: Hot-swap failed, reverting to bundled version";
+
...
+  }
+
+  LOG(INFO) << "orion: Hot-swap successful! Now running version "
...
+  orion_metrics::OrionMetrics::Log("server.ota.success",
+                                           std::move(props));
+
+  ResetState();
+}
+
+base::Version OrionServerUpdater::GetCurrentVersion() {
...
+}
+
+void OrionServerUpdater::InvalidateDownloadedVersion() {
+  LOG(WARNING) << "orion: Invalidating downloaded version, "
...
+                LOG(ERROR) << "orion: Failed to delete versions directory: "
+                           << versions_dir;
+              }
+            }
+          },
...
+}
+
+base::FilePath OrionServerUpdater::GetExecutionDir() const {
+  base::FilePath user_data_dir;
+  if (!base::PathService::Get(chrome::DIR_USER_DATA, &user_data_dir)) {
+    return base::FilePath();
+  }
+  return user_data_dir.Append(FILE_PATH_LITERAL(".orion"));
+}
+
+base::FilePath OrionServerUpdater::GetVersionsDir() const {
+  return GetExecutionDir().AppendASCII(kVersionsDirectoryName);
+}
+
+base::FilePath OrionServerUpdater::GetVersionDir(
+    const base::Version& version) const {
+  return GetVersionsDir().AppendASCII(version.GetString());
+}
+
+base::FilePath OrionServerUpdater::GetPendingUpdateDir() const {
+  return GetExecutionDir().AppendASCII(kPendingUpdateDirectoryName);
+}
+
+base::FilePath OrionServerUpdater::GetBundledBinaryPath() const {
+  // Delegate to manager's existing logic
+  return manager_->GetOrionServerExecutablePath();
+}
+
+base::FilePath OrionServerUpdater::GetBundledResourcesPath() const {
+  return manager_->GetOrionServerResourcesPath();
+}
+
+base::FilePath OrionServerUpdater::GetDownloadedBinaryPath(
+    const base::Version& version) const {
...
+}
+
+base::FilePath OrionServerUpdater::GetDownloadedResourcesPath(
+    const base::Version& version) const {
+  return GetVersionDir(version).Append(FILE_PATH_LITERAL("resources"));
+}
+
+base::FilePath OrionServerUpdater::GetBestServerBinaryPath() {
...
+}
+
+base::FilePath OrionServerUpdater::GetBestServerResourcesPath() {
+  // Use cached versions to avoid blocking I/O
+  base::Version downloaded = cached_downloaded_version_;
+  base::Version bundled = cached_bundled_version_;
+
...
+}
+
+void OrionServerUpdater::CleanupPendingUpdate() {
+  base::FilePath pending_dir = GetPendingUpdateDir();
...
+}
+
+void OrionServerUpdater::CleanupOldVersions() {
+  base::FilePath versions_dir = GetVersionsDir();
+
+  base::ThreadPool::PostTask(
+      FROM_HERE, {base::MayBlock()},
...
+              LOG(INFO) << "orion: Cleaning up old version: "
...
+              orion_metrics::OrionMetrics::Log("server.ota.cleanup",
+                                                       std::move(props));
+            }
+          },
...
+}
+
+void OrionServerUpdater::OnError(const std::string& stage,
+                                     const std::string& error) {
+  LOG(ERROR) << "orion: Update error at " << stage << ": " << error;
+
+  base::DictValue props;
...
+  }
+  orion_metrics::OrionMetrics::Log("server.ota.error",
+                                           std::move(props));
+
...
+            [](base::FilePath dir) {
+              if (base::PathExists(dir)) {
+                LOG(INFO) << "orion: Cleaning up failed version: " << dir;
+                base::DeletePathRecursively(dir);
+              }
+            },
+            version_dir));
...
+}
+
+void OrionServerUpdater::ResetState() {
+  state_ = State::kIdle;
