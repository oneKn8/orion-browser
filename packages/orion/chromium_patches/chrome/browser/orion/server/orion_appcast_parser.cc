diff --git a/chrome/browser/orion/server/orion_appcast_parser.cc b/chrome/browser/orion/server/orion_appcast_parser.cc
new file mode 100644
index 0000000000000..dd30f75d414ec
--- /dev/null
+++ b/chrome/browser/orion/server/orion_appcast_parser.cc
@@ -0,0 +1,202 @@
...
+
+// static
+std::optional<AppcastItem> OrionAppcastParser::ParseLatestItem(
...
+
+// static
+std::vector<AppcastItem> OrionAppcastParser::ParseAllItems(
+    const std::string& xml) {
+  std::vector<AppcastItem> items;
+
+  XmlReader reader;
+  if (!reader.Load(xml)) {
+    LOG(WARNING) << "orion: Failed to load appcast XML";
+    return items;
+  }
+
...
+        } else {
+          LOG(WARNING) << "orion: Skipping invalid appcast item (no valid "
...
+  }
+
+  VLOG(1) << "orion: Parsed " << items.size() << " appcast items";
+  return items;
+}
+
+}  // namespace orion_server
