diff --git a/chrome/browser/orion/server/orion_server_proxy.cc b/chrome/browser/orion/server/orion_server_proxy.cc
new file mode 100644
index 0000000000000..17560c9a2c55a
--- /dev/null
+++ b/chrome/browser/orion/server/orion_server_proxy.cc
@@ -0,0 +1,225 @@
...
+  return net::DefineNetworkTrafficAnnotation("orion_mcp_proxy", R"(
+    semantics {
+      sender: "Orion MCP Proxy"
+      description:
...
+        "Internal proxy for Orion MCP server functionality."
+    })");
+}
+
...
+}  // namespace
+
+OrionServerProxy::OrionServerProxy() = default;
+
+OrionServerProxy::~OrionServerProxy() {
+  Stop();
+}
+
+bool OrionServerProxy::Start(
+    int port,
+    std::unique_ptr<network::PendingSharedURLLoaderFactory> pending_factory) {
+  if (server_) {
+    LOG(WARNING) << "orion: Proxy already started on port " << bound_port_;
+    return false;
+  }
+
...
+                                                        kBackLog);
+  if (result != net::OK) {
+    LOG(ERROR) << "orion: Proxy failed to bind 0.0.0.0:" << port
...
+  bound_port_ = port;
+
+  LOG(INFO) << "orion: MCP proxy listening on 0.0.0.0:" << bound_port_;
+  return true;
+}
+
+void OrionServerProxy::Stop() {
+  pending_loaders_.clear();
+  if (server_) {
+    LOG(INFO) << "orion: Stopping MCP proxy on port " << bound_port_;
+    server_.reset();
+    bound_port_ = 0;
+  }
...
+}
+
+void OrionServerProxy::SetBackendPort(int port) {
+  backend_port_ = port;
+  LOG(INFO) << "orion: Proxy backend port set to " << port;
+}
+
+void OrionServerProxy::SetAllowRemote(bool allow) {
+  allow_remote_ = allow;
+  LOG(INFO) << "orion: Proxy allow_remote set to "
+            << (allow ? "true" : "false");
+}
+
+void OrionServerProxy::OnConnect(int connection_id) {}
+
+void OrionServerProxy::OnHttpRequest(
+    int connection_id,
...
+}
+
+void OrionServerProxy::OnWebSocketRequest(
+    int connection_id,
+    const net::HttpServerRequestInfo& info) {
+  server_->Close(connection_id);
+}
+
+void OrionServerProxy::OnWebSocketMessage(int connection_id,
+                                               std::string data) {
+  server_->Close(connection_id);
+}
+
+void OrionServerProxy::OnClose(int connection_id) {
+  pending_loaders_.erase(connection_id);
+}
+
+void OrionServerProxy::ForwardRequest(
+    int connection_id,
+    const net::HttpServerRequestInfo& info) {
...
+      base::BindOnce(&OrionServerProxy::OnBackendResponse,
+                     base::Unretained(this), connection_id),
+      kMaxResponseBodySize);
+}
+
+void OrionServerProxy::OnBackendResponse(
+    int connection_id,
