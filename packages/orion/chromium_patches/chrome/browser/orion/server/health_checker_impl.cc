diff --git a/chrome/browser/orion/server/health_checker_impl.cc b/chrome/browser/orion/server/health_checker_impl.cc
new file mode 100644
index 0000000000000..73ef5eaf9e8e3
--- /dev/null
+++ b/chrome/browser/orion/server/health_checker_impl.cc
@@ -0,0 +1,145 @@
...
+      net::DefineNetworkTrafficAnnotation("orion_health_check", R"(
+        semantics {
+          sender: "Orion Server Manager"
+          description:
+            "Checks if the Orion MCP server is healthy by querying its "
+            "/health endpoint."
...
+            "Internal health check for Orion server functionality."
+        })");
+
...
+  // Create network traffic annotation
+  net::NetworkTrafficAnnotationTag traffic_annotation =
+      net::DefineNetworkTrafficAnnotation("orion_shutdown_request", R"(
+        semantics {
+          sender: "Orion Server Manager"
+          description:
+            "Requests graceful shutdown of the Orion server via POST to "
+            "/shutdown endpoint."
...
+          setting: "This feature cannot be disabled by settings."
+          policy_exception_justification:
+            "Internal shutdown request for Orion server functionality."
+        })");
+
+  // Create resource request
+  auto resource_request = std::make_unique<network::ResourceRequest>();
...
+    LOG(WARNING) << "orion: HTTP request failed - HTTP " << response_code
