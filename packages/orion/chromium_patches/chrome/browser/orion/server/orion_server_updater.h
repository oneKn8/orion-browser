diff --git a/chrome/browser/orion/server/orion_server_updater.h b/chrome/browser/orion/server/orion_server_updater.h
new file mode 100644
index 0000000000000..a7edcdd9d98ee
--- /dev/null
+++ b/chrome/browser/orion/server/orion_server_updater.h
@@ -0,0 +1,165 @@
+// Copyright 2024 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#ifndef CHROME_BROWSER_ORION_SERVER_ORION_SERVER_UPDATER_H_
+#define CHROME_BROWSER_ORION_SERVER_ORION_SERVER_UPDATER_H_
+
+#include <memory>
+#include <optional>
+#include <string>
...
+
+// Manages automatic updates for the Orion server binary.
+//
+// Update flow:
...
+class OrionServerUpdater : public orion::ServerUpdater {
+ public:
+  explicit OrionServerUpdater(orion::OrionServerManager* manager);
+  ~OrionServerUpdater() override;
+
+  OrionServerUpdater(const OrionServerUpdater&) = delete;
+  OrionServerUpdater& operator=(const OrionServerUpdater&) = delete;
+
...
+  void ResetState();
+
+  raw_ptr<orion::OrionServerManager> manager_;
+
...
+
+  base::WeakPtrFactory<OrionServerUpdater> weak_factory_{this};
+};
+
+}  // namespace orion_server
+
+#endif  // CHROME_BROWSER_ORION_SERVER_ORION_SERVER_UPDATER_H_
