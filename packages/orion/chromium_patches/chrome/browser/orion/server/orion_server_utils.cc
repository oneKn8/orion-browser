diff --git a/chrome/browser/orion/server/orion_server_utils.cc b/chrome/browser/orion/server/orion_server_utils.cc
new file mode 100644
index 0000000000000..8b3137bf24e8a
--- /dev/null
+++ b/chrome/browser/orion/server/orion_server_utils.cc
@@ -0,0 +1,518 @@
...
+                      bool allow_reuse) {
+  LOG(INFO) << "orion: Finding port starting from " << starting_port;
+
...
+        LOG(INFO) << "orion: Port " << starting_port
+                  << " was in use or excluded, using " << port_to_try
+                  << " instead";
+      } else {
+        LOG(INFO) << "orion: Using port " << port_to_try;
+      }
+      return port_to_try;
+    }
+  }
+
+  LOG(WARNING) << "orion: Could not find available port after "
...
+      user_data_dir.Append(FILE_PATH_LITERAL(".orion"));
+
+  if (!base::PathExists(exec_dir)) {
+    if (!base::CreateDirectory(exec_dir)) {
+      LOG(ERROR) << "orion: Failed to create execution directory: "
+                 << exec_dir;
+      return base::FilePath();
+    }
+  }
...
+    LOG(WARNING) << "orion: Invalid state file format";
+    return std::nullopt;
+  }
+
...
+    LOG(WARNING) << "orion: State file missing required fields";
+    return std::nullopt;
+  }
+
+  ServerState state;
...
+
+  LOG(INFO) << "orion: Read state file - PID: " << state.pid
...
+    LOG(ERROR) << "orion: Failed to serialize state to JSON";
+    return false;
+  }
+
+  if (!base::WriteFile(state_path, json_output.value())) {
+    LOG(ERROR) << "orion: Failed to write state file: " << state_path;
+    return false;
+  }
+
+  LOG(INFO) << "orion: Wrote state file - PID: " << state.pid
+            << ", creation_time: " << state.creation_time;
+  return true;
+}
+
...
+    LOG(WARNING) << "orion: Failed to delete state file: " << state_path;
+    return false;
+  }
+
+  LOG(INFO) << "orion: Deleted state file";
+  return true;
+}
+
+// =============================================================================
...
+    }
+    PLOG(WARNING) << "orion: Failed to send SIGTERM to PID " << pid;
+    return false;
+  }
+
...
+    }
+    LOG(ERROR) << "orion: Failed to open process " << pid
...
+    DWORD error = GetLastError();
+    LOG(ERROR) << "orion: Failed to terminate process " << pid
+               << ", error: " << error;
+    return false;
+  }
+
