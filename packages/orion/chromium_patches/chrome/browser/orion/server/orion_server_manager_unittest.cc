diff --git a/chrome/browser/orion/server/orion_server_manager_unittest.cc b/chrome/browser/orion/server/orion_server_manager_unittest.cc
new file mode 100644
index 0000000000000..8f97e0b97467f
--- /dev/null
+++ b/chrome/browser/orion/server/orion_server_manager_unittest.cc
@@ -0,0 +1,497 @@
...
+namespace {
+
+class OrionServerManagerTest : public testing::Test {
+ protected:
+  void SetUp() override {
+    orion_server::RegisterLocalStatePrefs(prefs_.registry());
+
...
+
+  raw_ptr<OrionServerManager> manager_ = nullptr;
+};
+
+// =============================================================================
+// Health Check Tests
+// =============================================================================
+
+TEST_F(OrionServerManagerTest, HealthCheckPass_NoRestart) {
...
+}
+
+TEST_F(OrionServerManagerTest, HealthCheckFail_TriggersRestart) {
...
+}
+
+TEST_F(OrionServerManagerTest, HealthCheckPass_DoesNotRestart) {
+  manager_->SetRunningForTesting(true);
+
...
+// =============================================================================
+
+TEST_F(OrionServerManagerTest, StopCallsUpdaterStop) {
+  EXPECT_CALL(*updater_, Stop()).Times(1);
+  manager_->Stop();
+}
+
+TEST_F(OrionServerManagerTest, GetBinaryPathUsesUpdater) {
+  base::FilePath expected_path("/custom/binary/path");
+  EXPECT_CALL(*updater_, GetBestServerBinaryPath())
+      .WillOnce(Return(expected_path));
+}
+
+TEST_F(OrionServerManagerTest, GetResourcesPathUsesUpdater) {
...
+// =============================================================================
+
+TEST_F(OrionServerManagerTest, LoadsPortsFromPrefs) {
+  prefs_.SetInteger(orion_server::kCDPServerPort, 8000);
+  prefs_.SetInteger(orion_server::kProxyPort, 8100);
+
+  auto process_controller =
+      std::make_unique<NiceMock<MockProcessController>>();
+  auto state_store = std::make_unique<NiceMock<MockServerStateStore>>();
...
+
+  auto* manager = new OrionServerManager(
+      std::move(process_controller), std::move(state_store),
+      std::move(health_checker), std::move(updater), &prefs_);
+
+  manager->Shutdown();
+}
+
+TEST_F(OrionServerManagerTest, DefaultPortsWhenPrefsEmpty) {
+  EXPECT_EQ(orion_server::kDefaultCDPPort,
+            prefs_.GetInteger(orion_server::kCDPServerPort));
+
+  auto process_controller =
+      std::make_unique<NiceMock<MockProcessController>>();
+  auto state_store = std::make_unique<NiceMock<MockServerStateStore>>();
...
+}
+
+TEST_F(OrionServerManagerTest, MigratesOldMCPPortToProxy) {
+  // Set old pref (simulates pre-upgrade state)
+  prefs_.SetInteger(orion_server::kMCPServerPort, 9200);
+  // Ensure new proxy pref is at default (0, meaning not yet migrated)
+  prefs_.SetInteger(orion_server::kProxyPort, 0);
+
+  base::test::ScopedCommandLine scoped_command_line;
+  scoped_command_line.GetProcessCommandLine()->AppendSwitch(
+      orion::kDisableServer);
+
+  auto process_controller =
+      std::make_unique<NiceMock<MockProcessController>>();
+  auto state_store = std::make_unique<NiceMock<MockServerStateStore>>();
...
+}
+
+TEST_F(OrionServerManagerTest, AllowRemoteInMCPPref) {
+  prefs_.SetBoolean(orion_server::kAllowRemoteInMCP, true);
+
+  base::test::ScopedCommandLine scoped_command_line;
+  scoped_command_line.GetProcessCommandLine()->AppendSwitch(
+      orion::kDisableServer);
...
+// =============================================================================
+
+TEST_F(OrionServerManagerTest, HandlesNullPrefs) {
+  auto process_controller =
+      std::make_unique<NiceMock<MockProcessController>>();
+  auto state_store = std::make_unique<NiceMock<MockServerStateStore>>();
+  auto health_checker = std::make_unique<NiceMock<MockHealthChecker>>();
...
+// =============================================================================
+
+TEST_F(OrionServerManagerTest, HandlesNullUpdater) {
+  auto process_controller =
+      std::make_unique<NiceMock<MockProcessController>>();
+  auto state_store = std::make_unique<NiceMock<MockServerStateStore>>();
+  auto health_checker = std::make_unique<NiceMock<MockHealthChecker>>();
...
+// =============================================================================
+
+TEST_F(OrionServerManagerTest, InitiallyNotRunning) {
+  EXPECT_FALSE(manager_->IsRunning());
+}
+
+TEST_F(OrionServerManagerTest, PortsInitiallyZero) {
...
+// =============================================================================
+
+TEST_F(OrionServerManagerTest, RestartForUpdate_FailsWhenAlreadyRestarting) {
...
+// =============================================================================
+
+TEST_F(OrionServerManagerTest, TerminateUsesProcessController) {
...
+// =============================================================================
+
+TEST_F(OrionServerManagerTest, InvalidatesVersionOnFallback) {
+  ON_CALL(*process_controller_, Launch(_))
+      .WillByDefault([](const ServerLaunchConfig&) {
+        LaunchResult result;
+        result.process = base::Process::Current();
...
+// =============================================================================
+
+TEST_F(OrionServerManagerTest, StopDeletesStateFile) {
+  manager_->SetRunningForTesting(true);
+
...
+}
+
+TEST_F(OrionServerManagerTest, RecoverFromOrphan_NoStateFile) {
...
+}
+
+TEST_F(OrionServerManagerTest, RecoverFromOrphan_ProcessGone) {
...
+// =============================================================================
+
+TEST_F(OrionServerManagerTest, RestartSavesEphemeralPortsToPrefs) {
+  SetupSuccessfulLaunch();
+  manager_->SetRunningForTesting(true);
+
+  // Set initial known ports in prefs
+  prefs_.SetInteger(orion_server::kServerPort, 9200);
+  prefs_.SetInteger(orion_server::kExtensionServerPort, 9300);
+
...
+            prefs_.GetInteger(orion_server::kServerPort));
+  EXPECT_EQ(manager_->GetExtensionPort(),
+            prefs_.GetInteger(orion_server::kExtensionServerPort));
+  // Server and extension ports should be non-zero (resolved by FindAvailablePort)
+  EXPECT_NE(0, prefs_.GetInteger(orion_server::kServerPort));
+  EXPECT_NE(0, prefs_.GetInteger(orion_server::kExtensionServerPort));
+}
+
+TEST_F(OrionServerManagerTest, UpdateRestartSavesEphemeralPortsToPrefs) {
+  SetupSuccessfulLaunch();
+  manager_->SetRunningForTesting(true);
+
+  prefs_.SetInteger(orion_server::kServerPort, 9200);
