diff --git a/chrome/browser/orion/server/process_controller_impl.cc b/chrome/browser/orion/server/process_controller_impl.cc
new file mode 100644
index 0000000000000..a398856f672fd
--- /dev/null
+++ b/chrome/browser/orion/server/process_controller_impl.cc
@@ -0,0 +1,210 @@
...
+  instance.Set("orion_version", config.identity.orion_version);
...
+    LOG(ERROR) << "orion: Failed to serialize config to JSON";
+    return base::FilePath();
+  }
+
+  if (!base::WriteFile(config_path, json_output.value())) {
+    LOG(ERROR) << "orion: Failed to write config file: " << config_path;
+    return base::FilePath();
+  }
+
+  LOG(INFO) << "orion: Wrote config to " << config_path;
+  return config_path;
+}
+
+}  // namespace
...
+    LOG(WARNING) << "orion: Binary not found at " << actual_exe_path
...
+
+    if (!base::PathExists(actual_exe_path)) {
+      LOG(ERROR) << "orion: Bundled binary also not found at: "
+                 << actual_exe_path;
+      return result;
+    }
+  }
...
+    LOG(ERROR) << "orion: Execution directory path is empty";
+    return result;
+  }
+
+  // Ensure execution directory exists (blocking I/O)
+  if (!base::CreateDirectory(config.paths.execution)) {
+    LOG(ERROR) << "orion: Failed to create execution directory at: "
...
+    LOG(ERROR) << "orion: Failed to write config file, aborting launch";
+    return result;
+  }
+
...
+  }
+
+  LOG(INFO) << "orion: Terminating process with SIGKILL (PID: "
...
+      LOG(INFO) << "orion: Process killed successfully";
+    } else {
+      LOG(WARNING) << "orion: WaitForExit failed";
+    }
+  } else {
+    LOG(INFO) << "orion: SIGKILL sent (not waiting for exit)";
+  }
+#else
+  // Windows: Terminate with wait parameter
+  bool terminated = process->Terminate(0, wait);
+  if (terminated) {
+    LOG(INFO) << "orion: Process terminated successfully";
+  } else {
+    LOG(ERROR) << "orion: Failed to terminate process";
+  }
+#endif
+}
+
...
+  }
+
+  LOG(INFO) << "orion: Waiting for process exit (PID: " << process->Pid()
+            << ", timeout: " << timeout.InSeconds() << "s)";
+
+  bool exited = process->WaitForExitWithTimeout(timeout, exit_code);
+  if (exited) {
+    LOG(INFO) << "orion: Process exited with code " << *exit_code;
+  } else {
+    LOG(INFO) << "orion: Process did not exit within timeout";
+  }
+  return exited;
+}
+
