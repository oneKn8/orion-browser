diff --git a/chrome/browser/orion/server/orion_appcast_parser_unittest.cc b/chrome/browser/orion/server/orion_appcast_parser_unittest.cc
new file mode 100644
index 0000000000000..ceb483dee35c6
--- /dev/null
+++ b/chrome/browser/orion/server/orion_appcast_parser_unittest.cc
@@ -0,0 +1,389 @@
+// Copyright 2024 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#include "chrome/browser/orion/server/orion_appcast_parser.h"
+
...
+// =============================================================================
+
+TEST(OrionAppcastParserTest, ParsesValidAppcastWithSingleItem) {
...
+  )";
+
+  auto item = OrionAppcastParser::ParseLatestItem(kValidXml);
+
...
+}
+
+TEST(OrionAppcastParserTest, ParsesMultipleEnclosuresPerItem) {
...
+  )";
+
+  auto item = OrionAppcastParser::ParseLatestItem(kMultiPlatformXml);
+
+  ASSERT_TRUE(item.has_value());
...
+}
+
+TEST(OrionAppcastParserTest, ParseAllItems_ReturnsMultipleVersions) {
...
+  )";
+
+  auto items = OrionAppcastParser::ParseAllItems(kMultiVersionXml);
+
...
+// =============================================================================
+
+TEST(OrionAppcastParserTest, ReturnsNulloptForInvalidXml) {
+  auto item = OrionAppcastParser::ParseLatestItem("not valid xml at all");
+  EXPECT_FALSE(item.has_value());
+}
+
+TEST(OrionAppcastParserTest, ReturnsNulloptForEmptyString) {
+  auto item = OrionAppcastParser::ParseLatestItem("");
+  EXPECT_FALSE(item.has_value());
+}
+
+TEST(OrionAppcastParserTest, ReturnsNulloptForEmptyChannel) {
...
+  )";
+
+  auto item = OrionAppcastParser::ParseLatestItem(kEmptyXml);
+  EXPECT_FALSE(item.has_value());
+}
+
+TEST(OrionAppcastParserTest, ReturnsNulloptForMissingVersion) {
...
+  )";
+
+  auto item = OrionAppcastParser::ParseLatestItem(kNoVersionXml);
+  EXPECT_FALSE(item.has_value());
+}
+
+TEST(OrionAppcastParserTest, HandlesItemWithNoEnclosures) {
...
+  )";
+
+  auto item = OrionAppcastParser::ParseLatestItem(kNoEnclosureXml);
+
...
+}
+
+TEST(OrionAppcastParserTest, ParseAllItems_ReturnsEmptyForInvalidXml) {
+  auto items = OrionAppcastParser::ParseAllItems("invalid xml");
+  EXPECT_TRUE(items.empty());
+}
+
+TEST(OrionAppcastParserTest, ParseAllItems_ReturnsEmptyForEmptyChannel) {
+  const char kEmptyXml[] = R"(
+    <rss xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle">
+      <channel></channel>
+    </rss>
+  )";
+
+  auto items = OrionAppcastParser::ParseAllItems(kEmptyXml);
+  EXPECT_TRUE(items.empty());
+}
+
+// =============================================================================
...
+// =============================================================================
+
+TEST(OrionAppcastParserTest, HandlesWhitespaceInVersion) {
...
+  )";
+
+  auto item = OrionAppcastParser::ParseLatestItem(kWhitespaceXml);
+
...
+}
+
+TEST(OrionAppcastParserTest, HandlesZeroLengthEnclosure) {
...
+  )";
+
+  auto item = OrionAppcastParser::ParseLatestItem(kZeroLengthXml);
+
+  ASSERT_TRUE(item.has_value());
+  ASSERT_EQ(1u, item->enclosures.size());
...
+}
+
+TEST(OrionAppcastParserTest, HandlesMissingSignature) {
...
+  )";
+
+  auto item = OrionAppcastParser::ParseLatestItem(kNoSigXml);
+
+  ASSERT_TRUE(item.has_value());
+  ASSERT_EQ(1u, item->enclosures.size());
