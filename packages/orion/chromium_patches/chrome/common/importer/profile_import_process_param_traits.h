...
+                    orion_importer::ImportedCookieEntry> {
+  static const std::string& host_key(
+      const orion_importer::ImportedCookieEntry& r) {
+    return r.host_key;
+  }
+
...
+  }
+
+  static int64_t expires_utc(const orion_importer::ImportedCookieEntry& r) {
...
+  }
+
+  static bool is_secure(const orion_importer::ImportedCookieEntry& r) {
+    return r.is_secure;
+  }
+
+  static bool is_httponly(const orion_importer::ImportedCookieEntry& r) {
...
+  }
+
+  static int32_t source_port(const orion_importer::ImportedCookieEntry& r) {
+    return r.source_port;
+  }
+
+  static bool is_persistent(const orion_importer::ImportedCookieEntry& r) {
+    return r.is_persistent;
+  }
+
+  static bool Read(chrome::mojom::ImportedCookieEntryDataView data,
+                   orion_importer::ImportedCookieEntry* out);
+};
+
 }  // namespace mojo
