#!/usr/bin/env python3
"""Build CLI - Modular build system for Orion"""
...
def main(
    ...
    """Orion Build System - Modular pipeline executor

    Build Orion using phase flags (auto-ordered), explicit modules, or configs.

    \b
    Phase Flags (Recommended - Auto-Ordered):
      orion build --setup --build --sign --package
      orion build --build --sign           # Skip setup
      orion build --package --sign         # Flags work in any order!

    \b
    Explicit Modules (Power Users):
      orion build --modules clean,compile,sign_macos

    \b
    Config Files (CI/CD):
      orion build --config release.yaml --arch arm64

    \b
    List Available:
      orion build --list                   # Show all modules and phases
    ...
    if options_provided > 1:
        ...
        log_error("Examples:")
        log_error("  orion build --setup --build --sign")
        log_error("  orion build --modules clean,compile")
        log_error("  orion build --config release.yaml")
        raise typer.Exit(1)
    ...
    if has_flags:
        ...
        if prep:
            ...
            log_warning("⚠️  --prep does NOT apply series_patches. Run 'orion build -m series_patches' separately if needed.")
        if build:
    ...
    log_info(f"📍 Build offset: {ctx.orion_build_offset}")
