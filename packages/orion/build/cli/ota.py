#!/usr/bin/env python3
"""OTA CLI - Over-The-Air update automation for Orion"""
...
server_app = typer.Typer(
    help="Orion Server OTA commands",
    pretty_exceptions_enable=False,
    pretty_exceptions_show_locals=False,
)
...
def server_release(
    ...
    """Release Orion Server OTA update
    ...
    Full Release (all platforms):
      orion ota server release --version 0.0.69 --channel alpha

    \b
    Single Platform:
      orion ota server release --version 0.0.69 --platform darwin_arm64

    \b
    Multiple Platforms:
      orion ota server release --version 0.0.69 --platform darwin_arm64,darwin_x64
    """
    log_info(f"🚀 Orion Server OTA v{version}")
    log_info("=" * 70)
...
def server_release_appcast(
    ...
    Release alpha appcast:
      orion ota server release-appcast --channel alpha

    \b
    Release production appcast:
      orion ota server release-appcast --channel prod

    \b
    Release custom appcast file:
      orion ota server release-appcast --file /path/to/appcast.xml
    """
    if appcast_file:
        ...
        if not source_path.exists():
            ...
            log_error("Run 'orion ota server release' first to generate the appcast")
            raise typer.Exit(1)
    ...
    if upload_file_to_r2(r2_client, source_path, r2_key, env.r2_bucket):
        cdn_url = f"https://cdn.orion-browser.local/{r2_key}"
...
def test_signing(
    ...
    \b
    Example:
      orion ota test-signing /path/to/file.zip
    """
...
def server_main(ctx: typer.Context):
    """Orion Server OTA commands

    \b
    Release (upload artifacts):
      orion ota server release --version 0.0.36
    ...
    \b
    List Platforms:
      orion ota server list-platforms
    """
...
def main(ctx: typer.Context):
    """OTA update automation for Orion
