...
def main(
    ...
    Extract patches from commits:
      orion dev extract commit HEAD
      orion dev extract range HEAD~5 HEAD

    Apply patches:
      orion dev apply all
      orion dev apply feature llm-chat

    Manage features:
      orion dev feature list
      orion dev feature add my-feature HEAD
      orion dev feature show my-feature
    """
...
def apply_force(
    ...
    Examples:
        orion dev apply force -S /chromium
        orion dev apply force --reset-to base -S /chromium
    """
    ctx = create_build_context(state.chromium_src)
    if not ctx:
        raise typer.Exit(1)
...
def apply_changed(
    ...
    Examples:
        ...
        orion dev apply changed --commit 1c78477 --reset-to base -S /chromium

        # Apply patches changed in a range of commits
        orion dev apply changed --range-start HEAD~5 --range-end HEAD --reset-to base -S /chromium
    """
    ctx = create_build_context(state.chromium_src)
    if not ctx:
        raise typer.Exit(1)
...
def feature_add_update(
    ...
    Examples:
        orion dev feature add-update --name llm-chat --commit HEAD -d "feat: LLM chat panel"
        orion dev feature add-update -n api -c HEAD~3 -d "feat: orion API updates"
    """
    ctx = create_build_context(state.chromium_src)
    if not ctx:
        raise typer.Exit(1)
...
def annotate_cmd(
    ...
    Examples:
        orion dev annotate -S /path/to/chromium
        orion dev annotate llm-chat -S /path/to/chromium
    """
    ctx = create_build_context(state.chromium_src)
    if not ctx:
        raise typer.Exit(1)
