#!/usr/bin/env python3
"""Application signing and notarization module for Orion (macOS)"""
...
# Central list of Orion Server binaries we need to sign explicitly.
...
ORION_SERVER_BINARIES: Dict[str, Dict[str, str]] = {
    "orion_server": {
        "identifier_suffix": "orion_server",
        "options": "runtime",
        "entitlements": "orion-executable-entitlements.plist",
    },
    "codex": {
...
def get_orion_server_binary_info(component_path: Path) -> Optional[Dict[str, str]]:
    """Return metadata for known Orion Server binaries, if applicable."""
    name = component_path.stem.lower()
    return ORION_SERVER_BINARIES.get(name)
...
class MacOSSignModule(CommandModule):
    ...
    def execute(self, ctx: Context) -> None:
        log_info("=" * 70)
        log_info("🚀 Starting signing process for Orion...")
        log_info("=" * 70)
...
def find_components_to_sign(
    ...
    # Check both versioned and non-versioned paths for Orion Framework
    # Handle both release and debug framework names
    framework_names = [
        "Orion Framework.framework",
        "Orion Dev Framework.framework",
    ]
    ...
    for fw_name in framework_names:
        ...
        if fw_path.exists():
            ...
            if ctx and ctx.orion_chromium_version:
                versioned_path = join_paths(
                    fw_path, "Versions", ctx.orion_chromium_version
                )
    ...
    # Find all dylibs (check versioned path for Orion Framework libraries)
    for nxtscape_fw_path in nxtscape_framework_paths:
    ...
    # Find Orion Server binaries
    orion_server_dir = join_paths(app_path, "Contents", "Resources", "OrionServer")
    if orion_server_dir.exists():
        for item in orion_server_dir.rglob("*"):
            if item.is_file() and not item.suffix and os.access(item, os.X_OK):
                components["executables"].append(item)
...
def get_identifier_for_component(
    component_path: Path, base_identifier: str = "com.orion"
) -> str:
    ...
    # Orion Server binaries share the same entitlements/options but need unique identifiers.
    orion_server_info = get_orion_server_binary_info(component_path)
    if orion_server_info:
        suffix = orion_server_info.get("identifier_suffix", component_path.stem)
    ...
    if component_path.suffix == ".framework":
        if name == "Orion Framework" or name == "Orion Dev Framework":
...
def get_signing_options(component_path: Path) -> str:
    ...
    # Known Orion Server binaries share the same relaxed options.
    orion_server_info = get_orion_server_binary_info(component_path)
    if orion_server_info:
        return orion_server_info.get("options", "runtime")
...
def sign_all_components(
    ...
    if components["executables"]:
        ...
        for exe in components["executables"]:
            ...
            entitlements = None
            orion_server_info = get_orion_server_binary_info(exe)
            if orion_server_info:
                entitlements_name = orion_server_info.get("entitlements")
    ...
    # 6. Sign frameworks (except the main Orion Framework)
    if components["frameworks"]:
        log_info("\n🔏 Signing frameworks...")
        # Sort to sign Sparkle.framework before Orion Framework.framework
    ...
    main_exe_names = ["Orion", "Orion Dev"]
    main_exe = None
    ...
    if not sign_component(main_exe, certificate_name, "com.orion.Orion"):
        return False

    # 8. Finally sign the app bundle
    log_info("\n🔏 Signing application bundle...")
    requirements = (
        '=designated => identifier "com.orion.Orion" and '
...
def sign_universal(contexts: List[Context]) -> bool:
    ...
    universal_app_path = join_paths(universal_dir, contexts[0].ORION_APP_NAME)
