#!/usr/bin/env python3
"""Windows signing module for Orion"""
...
ORION_SERVER_BINARIES: List[str] = [
    "orion_server.exe",
    "codex.exe",
    "bun.exe",
]
...
class WindowsSignModule(CommandModule):
    ...
    def _sign_executables(self, build_output_dir: Path, env: EnvConfig) -> None:
        ...
        binaries_to_sign_first.extend(get_orion_server_binary_paths(build_output_dir))
...
def get_orion_server_binary_paths(build_output_dir: Path) -> List[Path]:
    """Return absolute paths to Orion Server binaries for signing."""
    server_dir = build_output_dir / "OrionServer" / "default" / "resources" / "bin"
    return [server_dir / binary for binary in ORION_SERVER_BINARIES]
