#!/usr/bin/env python3
"""Standard single-architecture build module for Orion"""
...
class CompileModule(CommandModule):
    produces = ["built_app"]
    requires = []
    description = "Build Orion using autoninja"

    def validate(self, ctx: Context) -> None:
        if not ctx.chromium_src.exists():
            raise ValidationError(f"Chromium source not found: {ctx.chromium_src}")

        if not ctx.orion_chromium_version:
            raise ValidationError("Orion chromium version not set")
    ...
    def execute(self, ctx: Context) -> None:
        log_info("\n🔨 Building Orion (this will take a while)...")
    ...
    def _create_version_file(self, ctx: Context) -> None:
        parts = ctx.orion_chromium_version.split(".")
        if len(parts) != 4:
            log_warning(f"Invalid version format: {ctx.orion_chromium_version}")
            return
        ...
        log_info(f"Created VERSION file: {ctx.orion_chromium_version}")
