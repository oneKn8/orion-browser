#!/usr/bin/env python3
"""Server OTA module for Orion Server binary updates"""
...
ARTIFACT_R2_KEY = "artifacts/server/latest/orion-server-resources-{target}.zip"


class ServerOTAModule(CommandModule):
    """OTA update module for Orion Server binaries
    ...
    requires = []
    description = "Create and upload Orion Server OTA update"
    ...
    def execute(self, context: Context) -> None:
        ctx = context
        log_info(f"\n🚀 Orion Server OTA v{self.version} ({self.channel})")
        log_info("=" * 70)
        ...
        for platform in platforms:
            ...
            zip_name = f"orion_server_{self.version}_{platform['name']}.zip"
        ...
        log_info("\nArtifact URLs:")
        for artifact in signed_artifacts:
            log_info(f"  https://cdn.orion-browser.local/server/{artifact.zip_path.name}")

        log_info(f"\nAppcast saved to: {appcast_path}")
        log_info("\n📋 Next step: Run 'orion ota server release-appcast' to make the release live")
