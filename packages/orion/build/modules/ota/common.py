...
SERVER_PLATFORMS = [
    {"name": "darwin_arm64", "binary": "orion-server-darwin-arm64", "target": "darwin-arm64", "os": "macos", "arch": "arm64"},
    {"name": "darwin_x64", "binary": "orion-server-darwin-x64", "target": "darwin-x64", "os": "macos", "arch": "x86_64"},
    {"name": "linux_arm64", "binary": "orion-server-linux-arm64", "target": "linux-arm64", "os": "linux", "arch": "arm64"},
    {"name": "linux_x64", "binary": "orion-server-linux-x64", "target": "linux-x64", "os": "linux", "arch": "x86_64"},
    {"name": "windows_x64", "binary": "orion-server-windows-x64.exe", "target": "windows-x64", "os": "windows", "arch": "x86_64"},
]
...
<rss xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" version="2.0">
  <channel>
    ...
    <description>Orion Server binary updates</description>
...
def find_server_binary(binaries_dir: Path, platform: dict) -> Optional[Path]:
    ...
    Supports two layouts:
      Flat:     {binaries_dir}/{binary_name}        (e.g., orion-server-darwin-arm64)
      Artifact: {binaries_dir}/{target}/resources/bin/orion_server[.exe]
    ...
    bin_name = "orion_server.exe" if platform["os"] == "windows" else "orion_server"
...
def parse_existing_appcast(appcast_path: Path) -> Optional[ExistingAppcast]:
    ...
    try:
        ...
        for enclosure in item.findall("enclosure"):
            ...
            # Extract platform from URL (e.g., orion_server_0.0.37_darwin_arm64.zip)
...
def generate_server_appcast(
    ...
    """
    if channel == "alpha":
        title = "Orion Server (Alpha)"
        appcast_url = "https://cdn.orion-browser.local/appcast-server.alpha.xml"
    else:
        title = "Orion Server"
        appcast_url = "https://cdn.orion-browser.local/appcast-server.xml"
    ...
    for artifact in final_artifacts:
        ...
        zip_filename = f"orion_server_{version}_{artifact.platform}.zip"
        url = f"https://cdn.orion-browser.local/server/{zip_filename}"
...
def create_server_zip(
    ...
    """Create zip with proper structure: resources/bin/orion_server
    ...
    try:
        ...
        target_name = "orion_server.exe" if is_windows else "orion_server"
