#!/usr/bin/env python3
"""Linux packaging module for Orion (AppImage and .deb)"""
...
class LinuxPackageModule(CommandModule):
    ...
    def validate(self, ctx: Context) -> None:
        ...
        chrome_binary = join_paths(out_dir, ctx.ORION_APP_NAME)
    ...
    def execute(self, ctx: Context) -> None:
        log_info(
            f"\n📦 Packaging {ctx.ORION_APP_BASE_NAME} {ctx.get_orion_chromium_version()} for Linux ({ctx.architecture})"
        )
...
def copy_browser_files(
    ...
    dirs_to_copy = ["locales", "MEIPreload", "OrionServer"]
    ...
    orion_path = Path(join_paths(target_dir, ctx.ORION_APP_NAME))
    if orion_path.exists():
        orion_path.chmod(0o755)
...
"""
    ...
    desktop_file = Path(join_paths(apps_dir, "orion.desktop"))
...
def copy_icon(ctx: Context, icons_dir: Path) -> bool:
    ...
    for size in [16, 22, 24, 32, 48, 64, 128, 256]:
        ...
        if icon_src.exists():
            icon_dest = Path(join_paths(icons_dir, f"{size}x{size}", "apps", "orion.png"))
...
def prepare_appdir(ctx: Context, appdir: Path) -> bool:
    ...
    app_root = join_paths(appdir, "opt", "orion")
    ...
    desktop_file = create_desktop_file(
        apps_dir, f"/opt/orion/{ctx.ORION_APP_NAME}"
    )
    ...
    if icon_src.exists():
        appdir_icon = Path(join_paths(appdir, "orion.png"))
...
THIS="$(readlink -f "${{0}}")"
HERE="$(dirname "${{THIS}}")"
export LD_LIBRARY_PATH="${{HERE}}"/opt/orion:$LD_LIBRARY_PATH
export CHROME_WRAPPER="${{THIS}}"
"${{HERE}}"/opt/orion/{ctx.ORION_APP_NAME} "$@"
"""
...
def create_launcher_script(ctx: Context, bin_dir: Path) -> None:
    """Create launcher script in /usr/bin/orion."""
    bin_dir.mkdir(parents=True, exist_ok=True)

    launcher_content = f"""#!/bin/sh
# Orion launcher script
export LD_LIBRARY_PATH=/usr/lib/orion:$LD_LIBRARY_PATH
exec /usr/lib/orion/{ctx.ORION_APP_NAME} "$@"
"""

    launcher_path = Path(join_paths(bin_dir, "orion"))
...
Recommends: apparmor
Maintainer: Orion Team <support@orion-browser.local>
Homepage: https://www.orion-browser.local/
Description: Orion - The open source agentic browser
 Orion is a privacy-focused web browser built on Chromium,
...
# Post-installation script for Orion
set -e

# Set SUID bit on chrome_sandbox for sandboxing support
if [ -f /usr/lib/orion/chrome_sandbox ]; then
    chmod 4755 /usr/lib/orion/chrome_sandbox
fi
...
if [ -d /etc/apparmor.d ] && command -v apparmor_parser >/dev/null 2>&1; then
    apparmor_parser -r -T -W /etc/apparmor.d/orion 2>/dev/null || true
fi

# Register as a selectable default browser
if [ "$1" = "configure" ]; then
    update-alternatives --install /usr/bin/x-www-browser x-www-browser /usr/bin/orion 40
    update-alternatives --install /usr/bin/gnome-www-browser gnome-www-browser /usr/bin/orion 40
fi
...
# Pre-removal script for Orion
set -e

# Unregister as default browser
if [ "$1" = "remove" ] || [ "$1" = "deconfigure" ]; then
    update-alternatives --remove x-www-browser /usr/bin/orion 2>/dev/null || true
    update-alternatives --remove gnome-www-browser /usr/bin/orion 2>/dev/null || true
fi

# Unload AppArmor profile before files are removed
if command -v apparmor_parser >/dev/null 2>&1 && [ -f /etc/apparmor.d/orion ]; then
    apparmor_parser -R /etc/apparmor.d/orion 2>/dev/null || true
fi
...
def create_apparmor_profile(ctx: Context, apparmor_dir: Path) -> None:
    ...
    profile_content = f"""# AppArmor profile for Orion
...
profile orion /usr/lib/orion/{ctx.ORION_APP_NAME} flags=(unconfined) {{
  userns,

  include if exists <local/orion>
}}
"""

    profile_path = Path(join_paths(apparmor_dir, "orion"))
...
def create_metainfo_file(ctx: Context, metainfo_dir: Path) -> None:
    ...
    and other AppStream-aware tools can display Orion in their catalogs.
    """
...
<component type="desktop-application">
  ...
  <name>Orion</name>
  <developer id="com.orion">
    <name>Orion Team</name>
  </developer>
  ...
  <url type="homepage">https://www.orion-browser.local/</url>
  <url type="bugtracker">https://github.com/oneKn8/orion-browser/issues</url>
  <url type="help">https://docs.orion-browser.local/</url>
  <description>
    <p>
      Orion is a privacy-focused web browser built on Chromium,
      designed for modern web browsing with AI capabilities.
...
"""
    ...
    metainfo_path = Path(join_paths(metainfo_dir, "orion.metainfo.xml"))
...
def prepare_debdir(ctx: Context, debdir: Path) -> bool:
    ...
    │   │   └── [all browser files]
    │   └── share/
    │       ├── applications/orion.desktop
    │       ├── icons/hicolor/{16..256}x{16..256}/apps/orion.png
    │       └── metainfo/orion.metainfo.xml
    """
    log_info("📁 Preparing .deb directory structure...")

    lib_dir = join_paths(debdir, "usr", "lib", "orion")
    ...
    # Create desktop file
    create_desktop_file(apps_dir, "/usr/bin/orion")
...
def package_appimage(ctx: Context, package_dir: Path) -> Optional[Path]:
    ...
    appdir = Path(join_paths(package_dir, f"{ctx.ORION_APP_BASE_NAME}.AppDir"))
    if appdir.exists():
        safe_rmtree(appdir)
...
def package_deb(ctx: Context, package_dir: Path) -> Optional[Path]:
    ...
    debdir = Path(join_paths(package_dir, f"{ctx.ORION_APP_BASE_NAME}_deb"))
    if debdir.exists():
        safe_rmtree(debdir)
