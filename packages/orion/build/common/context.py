...
class ArtifactRegistry:
    ...
    Example:
        ...
        artifacts.add("built_app", Path("/path/to/Orion.app"))
...
class BuildConfig:
    ...
    def _set_app_names(self):
        ...
        if IS_WINDOWS():
            ...
            self.ORION_APP_NAME = (
                f"{self.ORION_APP_BASE_NAME}{get_executable_extension()}"
            )
        elif IS_MACOS():
            self.CHROMIUM_APP_NAME = "Chromium.app"
            self.ORION_APP_NAME = f"{self.ORION_APP_BASE_NAME}.app"
        else:
            self.CHROMIUM_APP_NAME = "chrome"
            self.ORION_APP_NAME = self.ORION_APP_BASE_NAME.lower()
...
class Context:
    ...
    orion_build_offset: str = ""
    orion_chromium_version: str = ""
    semantic_version: str = ""  # e.g., "0.31.0" from resources/ORION_VERSION
    release_version: str = (
    ...
    ORION_APP_NAME: str = ""
    ORION_APP_BASE_NAME: str = "Orion"  # Base name without extension
    ...
    def __post_init__(self):
        ...
        self.build.ORION_APP_NAME = self.ORION_APP_NAME
        ...
        if not self.orion_build_offset:
            self.orion_build_offset = self._load_orion_build_offset(
                self.root_dir
            )

        # Load semantic version from resources/ORION_VERSION
        ...
        if self.chromium_version and self.orion_build_offset and version_dict:
            # Calculate new BUILD number by adding nxtscape_version to original BUILD
            new_build = int(version_dict["BUILD"]) + int(self.orion_build_offset)
            self.orion_chromium_version = f"{version_dict['MAJOR']}.{version_dict['MINOR']}.{new_build}.{version_dict['PATCH']}"

        # Sync versions with BuildConfig
        self.build.chromium_version = self.chromium_version
        self.build.orion_version = self.orion_build_offset
        self.build.orion_chromium_version = self.orion_chromium_version
    ...
    @staticmethod
    def _load_orion_build_offset(root_dir: Path) -> str:
        """Load orion build offset from config/ORION_BUILD_OFFSET"""
        version_file = join_paths(root_dir, "build", "config", "ORION_BUILD_OFFSET")
        if version_file.exists():
    ...
    def _load_semantic_version(root_dir: Path) -> str:
        ...
        """
        version_file = join_paths(root_dir, "resources", "ORION_VERSION")
        ...
        major = version_dict.get("ORION_MAJOR", "0")
        minor = version_dict.get("ORION_MINOR", "0")
        build = version_dict.get("ORION_BUILD", "0")
        patch = version_dict.get("ORION_PATCH", "0")
    ...
    def get_extensions_manifest_url(self) -> str:
        ...
        # return "https://cdn.orion-browser.local/extensions/update-manifest.xml"
        return "https://cdn.orion-browser.local/extensions/update-manifest.alpha.xml"
    ...
    def get_app_path(self) -> Path:
        ...
        For universal builds, checks if out/Default_universal/Orion.app exists
        ...
        if IS_MACOS():
            universal_app = join_paths(
                self.chromium_src, "out/Default_universal", self.ORION_APP_NAME
            )
        ...
        if self.build_type == "debug" and IS_MACOS():
            ...
            debug_app_name = f"{self.ORION_APP_BASE_NAME} Dev.app"
        ...
        return join_paths(self.chromium_src, self.out_dir, self.ORION_APP_NAME)
    ...
    def get_orion_chromium_version(self) -> str:
        """Get orion chromium version string"""
        return self.orion_chromium_version

    def get_orion_version(self) -> str:
        """Get orion version string (build offset)"""
        return self.orion_build_offset

    def get_semantic_version(self) -> str:
        """Get semantic version from resources/ORION_VERSION
    ...
    def get_sparkle_version(self) -> str:
        """Get Sparkle-compatible version from orion_chromium_version

        Sparkle uses BUILD.PATCH format for version comparison.
        Returns: e.g., "7231.69"
        """
        if not self.orion_chromium_version:
            raise ValueError("orion_chromium_version is not set")

        parts = self.orion_chromium_version.split(".")
        if len(parts) < 4:
            raise ValueError(
                f"Invalid orion_chromium_version format: {self.orion_chromium_version}"
            )
    ...
    def get_app_base_name(self) -> str:
        ...
        return self.ORION_APP_BASE_NAME
