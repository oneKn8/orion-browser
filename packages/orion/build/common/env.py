#!/usr/bin/env python3
"""
Environment variable configuration for Orion build system
...
def _load_dotenv_file():
    ...
    orion_root = get_package_root()
    project_root = orion_root.parent.parent  # repo root

    # Try loading .env from multiple locations (most specific first)
    env_locations = [
        orion_root / ".env",  # packages/orion/.env
...
class EnvConfig:
    ...
    def r2_bucket(self) -> str:
        """R2 bucket name (default: orion)"""
        return os.environ.get("R2_BUCKET", "orion")

    @property
    def r2_cdn_base_url(self) -> str:
        """CDN base URL for R2 artifacts (default: http://cdn.orion-browser.local)"""
        return os.environ.get("R2_CDN_BASE_URL", "http://cdn.orion-browser.local")
