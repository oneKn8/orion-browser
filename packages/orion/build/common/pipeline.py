#!/usr/bin/env python3
"""Pipeline validation for Orion build system"""
...
def show_available_modules(available_modules: Dict[str, Type[CommandModule]]) -> None:
    ...
    log_info("=" * 70)
    log_info("  orion build --modules clean,git_setup,configure,compile")
    log_info("  orion build --modules compile,sign_macos,package_macos")
    log_info("  orion build --config release.yaml")
    log_info("=" * 70 + "\n")
